...
#define MEX(arr, result) { \
    ll limit = max((ll)(arr).size() + 2, 6000LL); \
...
void solve() {
    ...
    while (a.size() > 1) {
        ...
        forn(j, a.size()) {
            hasZero = (a[j] == 0) ? true : hasZero;
            pos = (a[j] == 0 && hasZero) ? j : pos;
            if (hasZero) break;
        }
        ...
        if (!hasZero) {
            ...
            ll L = (pos < (ll)a.size() - 1) ? pos : pos - 1;
            ll R = (pos < (ll)a.size() - 1) ? pos + 1 : pos;
            ops.pb({L + 1, R + 1});
            vll seg;
            forr(k, L, R + 1) {
                seg.pb(a[k]);
