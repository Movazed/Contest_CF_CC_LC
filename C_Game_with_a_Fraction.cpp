...
stc ab : aa{
    ...
    template<class ff>
    static ff fn(ff x){ return x; }

    void go(void* vp) override{
        ll kk=0,ii1=0,jj1=0,ll1=0;
        ll* pk = reinterpret_cast<ll*>(&kk);
        ll* pi = reinterpret_cast<ll*>(&ii1);
        ll* pj = reinterpret_cast<ll*>(&jj1);
        ll* pl = reinterpret_cast<ll*>(&ll1);
        iin >> *pk;
        aut lm = [&](){
            fr(x,0,(ii)(*pk)){
                iin >> *pi >> *pj;
                *pl = (*pj/3)*2 + (2*((*pj)%3)+2)/3;
                (jj1>ii1 && ii1>=ll1)? (cout<<"Bob\n",(void)0): (cout<<"Alice\n",(void)0);
            }
        };
        lm();
        (void)vp;
...
int32_t main(){
    ...
    p->go(vp);
    delete reinterpret_cast<ab<ac>*>(p);
    re 0;
}