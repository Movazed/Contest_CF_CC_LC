...
#define EXECUTE_FAST std::ios_base::sync_with_stdio(false); std::cin.tie(NULL);
#define TEST_RUNNER int t; std::cin >> t; while (t--)
#define INT_64 long long
// Constants
mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
#define Movazed do { \
    INT_64 count_ones, count_twos; \
    cin >> count_ones >> count_twos; \
    INT_64 final_score; \
    if (2 * count_twos >= count_ones) { \
        final_score = 2 * (count_ones + count_twos) / 3; \
    } else { \
        final_score = 2 * count_twos + (count_ones - 2 * count_twos) / 3; \
    } \
    cout << final_score << "\n"; \
} while(0)
// Helper Functions
