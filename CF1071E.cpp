...
#define nuller if (!(cin >> n >> x >> y)) return;
// Constants
mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
...
void solve() {
    ...
    ll sp = 0, xmb = 0, ymb = 0, ez = 0, eo = 0, nz = 0, no = 0;

    fl(i, 0, n) {
        ll pi;
        cin >> pi;
        sp = sp + pi;
        (s[i] == '0') ? (nz++, xmb += (pi / 2 + 1), (pi % 2 == 0 ? ez++ : 0)) : (no++, ymb += (pi / 2 + 1), (pi % 2 == 0 ? eo++ : 0));
    }

    ll rx = x - xmb, ry = y - ymb, dl = (x + y) - sp;

    cout << ((dl < 0 || rx < 0 || ry < 0) ? "NO" : ((dl <= (nz > 0 ? (2 * rx + ez) : 0) + (no > 0 ? (2 * ry + eo) : 0)) ? "YES" : "NO")) << nl;
}
