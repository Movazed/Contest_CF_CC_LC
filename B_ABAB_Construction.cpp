...
#define lp(v,s,e,st) for(ii v=(s);v<(e);v+=(st))
#define lw(v) while(v--)
...
template<class tt>
struct ob : ab<tt> {
    tt go(tt v) override { return v; }
};

struct ap : bs {

    template<class tt>
    static tt tr(tt v){ return v; }
    ...
    void run() override {
        ...
        auto fn = function<void()>([&](){
            ...
            lp(jj,si,nn,2){
                ...
                auto p1 = reinterpret_cast<char*>(
                              static_cast<void*>(
                                  const_cast<char*>(&(*it2))
                              )
                          );

                ok = ok && !((*p0=='a' && *p1=='a') || (*p0=='b' && *p1=='b'));
...
int main(){
    ...
    ob<ii> oo;
    void* vp = reinterpret_cast<void*>(&oo);
    ab<ii>* apb = reinterpret_cast<ab<ii>*>(vp);
    apb->go(0);

    ap aa;
    bs* bb = reinterpret_cast<bs*>(
                static_cast<void*>(&aa)
             );
