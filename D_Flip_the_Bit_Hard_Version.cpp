...
struct sv {
    void operator()() {
        ...
        auto build_t = [&](void*)->ii {
            lp(i,1,n+1) t[i] = (a[i] != x);
            return 0;
        };

        auto build_d = [&](void*)->ii {
            lp(i,0,n+1) d[i] = (t[i] != t[i+1]);
            return 0;
        };

        by<decltype(build_t)> bt;
        by<decltype(build_d)> bd;

        bt.f((void*)&build_t, nullptr);
        bd.f((void*)&build_d, nullptr);
...
int32_t main() {
    ...
    ii t;
    iin >> t;
    lp(i,0,t) obj->g(nullptr, nullptr);
