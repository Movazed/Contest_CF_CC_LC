...
inline ll pw(ll b,ll e){
    ll r=1; b%=md;
    for(;e;e>>=1,b=b*b%md) if(e&1) r=r*b%md;
    return r;
}
inline ll inv(ll x){ return pw(x,md-2); }
inline ll fac(ii n){
    ll r=1;
    for(ii i=2;i<=n;i++) r=r*i%md;
    return r;
}

void sl(){
    ii n; cin>>n;vvi g(n+1);
    fl(i,n-1){
        ii u,v; iin>>u>>v;
    (*(&g[u])).pb(v);
    (*(&g[v])).pb(u);
    ...
    q.pb(1); vis[1]=1;
    for(ii h=0;h<(ii)q.size();h++){
        ii u=q[h];
        ord.pb(u);
        for(ii v:g[u]) if(!vis[v]){
            vis[v]=1;
            pa[v]=u;
            q.pb(v);
        }
    ...
    ll pr=1;
    fm(ll x:bs) pr = pr * (x*x%md) % md;
    as = as * pr % md;

    ll si=0;
    fm(ll x:bs) si = (si + inv(x)) % md; as = as * si % md;
