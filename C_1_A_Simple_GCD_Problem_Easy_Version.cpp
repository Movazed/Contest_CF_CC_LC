...
ll l2(ll a,ll b){ return (a==0||b==0?0:(a/std::gcd(a,b))*b); }

struct v0{
    virtual void r(void*,void*)=0;
    virtual ~v0() {}
};

struct v1: v0{
    void r(void* x,void* y) override {
        auto n = *(ii*)x;
        auto &o = *(vll*)y;
    }
};
...
struct s {
    ...
    ii go(){
       vll g(n-1);
        lp(i,0,n-1) g[i] = std::gcd(*(a.data() + i), *(a.data() + i + 1));

        ii m=0;
        lp(i,0,n){
            ll l = (i==0? g[0] : (i==n-1? g[n-2] : l2(g[i-1],g[i])));
            m += (l < a[i] ? 1 : 0);
        }
        return m;
    }
