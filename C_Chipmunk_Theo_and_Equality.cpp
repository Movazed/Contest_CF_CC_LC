...
struct bb:aa<t>{
    ...
    void f() override{
        ...
        vbo v(c.szz(),1);
        vll s(c.szz(),0);
        frr(size_t *i=reinterpret_cast<size_t*>(new size_t(0));*i<c.szz();++(*i))
            s[*i]=c[*i].scs;
        vector<cd>z;
        frr(size_t *i=reinterpret_cast<size_t*>(new size_t(0));*i<c.szz();++(*i))
            z.pb({c[*i].first,(ii)(*i)});
        ...
        ll an=-1;
        frr(size_t *i=reinterpret_cast<size_t*>(new size_t(0));*i<c.size();++(*i))
            if(v[*i])
                an==-1?an=s[*i]:an=min(an,s[*i]);
        pri<<an<<nl;
    }
};
