...
void ms(ii v){ par[v]=v, sz[v]=1, act[v]=1; }

ii fs(ii v){ re v==par[v]?v:par[v]=fs(par[v]); }

void us(ii a, ii b){
    a=fs(a), b=fs(b);
    if(a!=b){
        cur-=(1LL*sz[a]*(sz[a]+1))/2,
        cur-=(1LL*sz[b]*(sz[b]+1))/2,
        (sz[a]<sz[b]) && (swap(a,b),1),
        par[b]=a,
        sz[a]+=sz[b],
        cur+=(1LL*sz[a]*(sz[a]+1))/2;
    }
...
void sl(){
    ii n; iin>>n;
    vi p(n); fl(i,0,n-1) iin>>p[i];
    vi d(n-1); vvi g(n);
    fl(i,0,n-2) d[i]=abs(p[i]-p[i+1]), g[d[i]].pb(i);
    par.rez(n-1), sz.rez(n-1), act.rez(n-1);
    fll(all(par), 0);fll(all(sz), 0);fll(all(act), false);
    cur=0;vll ans(n);fr(k,n-1,1){
    for(ii idx : g[k]){ms(idx), cur+=1;
    (idx>0 && act[idx-1]) && (us(idx-1, idx),1);(idx<n-2 && act[idx+1]) && (us(idx, idx+1),1);}ans[k]=cur;}
    fl(k,1,n-1) (k==n-1 ? (void)(pri << ans[k]) : (void)(pri << ans[k] << " "));pri<<nl;
}
