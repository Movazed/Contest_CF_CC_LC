...
def fetch_input():
    ...
    # Read the length of the sequence from input, stripping extra whitespace
    sequence_length = int(sys.stdin.readline().strip().strip())
    
    # Read the sequence of numbers, strip all whitespace, and convert them to integers
    number_sequence = list(map(int, [x.strip() for x in sys.stdin.readline().strip().split()]))
    ...
    return sequence_length, number_sequence, high_tracker, low_tracker

def process_sequences():
    """Processes sequences and computes results."""
    MOD = 998244353
    # Read the number of test cases, stripping extra whitespace
    test_cases = int(sys.stdin.readline().strip().strip())
