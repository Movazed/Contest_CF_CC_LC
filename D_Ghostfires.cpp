...
#define sof sizeof
template<class t>
t rd(){ t x; cin>>x; return x; }

template<class t>
struct ab{ virtual t f(void*)=0; };

template<class t>
struct im:ab<t>{
    function<t(void*)> g;
    im(function<t(void*)> _g):g(_g){}
    t f(void* p){ return g(p); }
};
...
int32_t main(){
    ...
    im<void> ex([&](void*){
        wl(t--){
            ii r = rd<ii>(), g = rd<ii>(), b = rd<ii>();
            pich* p = reinterpret_cast<pich*>(operator new(sof(pich)*3));
            *reinterpret_cast<pich*>(p+0) = {r,'R'};
            *reinterpret_cast<pich*>(p+1) = {g,'G'};
            *reinterpret_cast<pich*>(p+2) = {b,'B'};
            sort(p,p+3);
            ii z = (*(p+0)).frs; word C = (*(p+0)).scs;
            ii y = (*(p+1)).frs; word B = (*(p+1)).scs;
            ii x = (*(p+2)).frs; word A = (*(p+2)).scs;
            line q;
            reinterpret_cast<line*>(&q)->revz(x+y+z);
            aut ap = [&](word c){
                (*reinterpret_cast<line*>(&q)).pb(c);
            };
            (x >= y + z)
            ?
            (
            ...
            pri<<(*reinterpret_cast<line*>(&q))<<nl;
            operator delete(p);
        }
    ...
    ab<void>* bp = reinterpret_cast<ab<void>*>(&ex);
    bp->f(nullptr);   
