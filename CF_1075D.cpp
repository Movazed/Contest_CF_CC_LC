...
#define anT_ an = (an * t) % md;vca  = (vca  * t) % c;
#define decss pri<<(vca==0 ? "-1" : tos(an))<<nl;
#define concord (ss[k-1]=='1' ? 2LL : (k-1))
#define md 1000000007LL


struct _V { virtual ll f(ll x){ re x; } };
void s(){
    ll n,c; 
    iin>>n>>c;line s; iin>>s;
    void* vp = (void*)&s;
    line& ss = *static_cast<line*>(vp);
    nuller ll an=1, vca=1;
    _V* v = new _V();
    fl(k,1,n){ll t = concord; anT_}
    delete v;
    decss
}
