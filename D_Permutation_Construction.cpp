...
#define vplli vector<pair<ll,ii>>
template<class t>
struct bx{
    virtual t& gt()=0;
    virtual ~bx(){}
...
struct rs:sv{
    ...
    void go() override{
        ...
        fr(i,1,n+1)
            iin>>*reinterpret_cast<ll*>(&a[i]);

        void* pw=new vplli(n);
        auto& w=cv<vplli>(pw);

        *reinterpret_cast<pair<ll,ii>*>(&w[n-1])={0,n};

        ll cs=0;

        rf(i,n-1,1){
            cs+=*reinterpret_cast<ll*>(&a[i]);
            *reinterpret_cast<pair<ll,ii>*>(&w[i-1])={cs,i};
        }

        sort(cp<vplli>(pw)->begin(),
             cp<vplli>(pw)->end(),
             [](auto& x,auto& y){
                return x.frs<y.frs?1:(x.frs>y.frs?0:x.scs<y.scs);
             });
        ...
        fr(i,0,n)
            *reinterpret_cast<ii*>(&p[w[i].scs])=i+1;
