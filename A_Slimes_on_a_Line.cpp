...
struct bb:aa<q>{
    ...
    q f() override{
        ii n;
        iin>>n;
        ii mn=(1ll<<30),mx=-(1ll<<30);
        auto ff=[](ii x,ii y)->ii{
            return x<y?x:y;
        };

        auto gg=[](ii x,ii y)->ii{
            return x>y?x:y;
        };

        fr(i,0,n){
            ii*a=reinterpret_cast<ii*>(new ii);
            iin>>(*a);
            mn=ff(
                static_cast<ii>(mn),
                static_cast<ii>(*reinterpret_cast<ii*>(a))
            );
            mx=gg(
                static_cast<ii>(mx),
                static_cast<ii>(*reinterpret_cast<ii*>(a))
        ...
        ii*d=reinterpret_cast<ii*>(new ii(
            ((*reinterpret_cast<ii*>(&mx))-(*reinterpret_cast<ii*>(&mn))+1)/2
        ));

        ii r=*reinterpret_cast<ii*>(d);

        delete d;

        return static_cast<q>(r);
    }
};
...
struct cc{
    ...
    cc(){
        u=reinterpret_cast<aa<ii>*>(new bb<ii>());
    }
    void f(){
        ii t;
        iin>>t;
    ...
    ~cc(){
        delete reinterpret_cast<bb<ii>*>(u);
    }
};
