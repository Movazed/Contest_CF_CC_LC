...
struct ee{ii v,i;};
template<class t> struct vv{virtual bool f(t,t)=0; virtual ~vv(){};};
template<class t> struct ww:vv<t>{bool f(t a,t b){re a.v!=b.v?a.v<b.v:a.i<b.i;}};


template<class t>
bool ck(ii n,ii k,const vector<t>&a){
    const t* p=(const t*)a.data();
    ii l=lower_bound(all(a),p[0].v+k,[](const t&x,ii y){re x.v<y;})-a.begin();
    ii r=upper_bound(all(a),p[n-1].v-k,[](ii x,const t&y){re x<y.v;})-a.begin()-1;
    fl(i,n) if(i>r&&i<l&&a[i].i!=i) re 0;
    re 1;
}

struct slv{virtual void r()=0; virtual ~slv(){};};
struct ms:slv{
    void s(){
        ii n; iin>>n;
        vi b(n);
        vector<ee>a(n);
        bool t=1;
        fl(i,n){iin>>b[i]; a[i]={b[i],i}; t&=(i?b[i]>=b[i-1]:1);}
        t?(void)(pri<<-1<<nl):(void)0;
        if(t) re;
        vv<ee>*q=new ww<ee>();
        sort(a.begin(),a.end(),[&](ee x,ee y){re q->f(x,y);});
        ll l=1,r=1000000007,an=0;
        wl(l<=r){ll m=l+(r-l)/2; ck<ee>(n,(ii)m,a)?(an=m,l=m+1):(r=m-1);}
        pri<<an<<nl;
        delete q;
    }
    void r(){s();}
...
int32_t main(){
    nfio
    static ms* a=new ms();
    slv* b=reinterpret_cast<slv*>(a);
    ii t; iin>>t;
    fl(_,t) static_cast<ms*>(b)->r();
    delete a;
}