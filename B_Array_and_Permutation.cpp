...
#define fe(it,c) for(auto it=(c).begin();it!=(c).end();++it)
#define fo(i,a,b) for(ii i=(a);i<(b);++i)
#define yeno ok ? pri<<"YES\n" : cout<<"NO\n";

stc bb{
    virtual ii go(void*,void*)=0;
    virtual ~bb(){}
};
stc bc:bb{
    ii (*fn)(void*,void*);
    ii go(void* a,void* b){ return fn(a,b); }
};
stc dd{
    vi* ps;
    vi* ar;
...
ii ck(void* x, void* y){
    t* p = static_cast<t*>(x);
    ii* i = static_cast<ii*>(y);
    return (*p->ps)[ (*p->ar)[*i] ] <= (*p->ps)[ (*p->ar)[*i+1] ];
}
...
void sv(){
    ...
    bc v;
    v.fn = &ck<dd>;
    bb* b = static_cast<bb*>(&v);
    ii ok=1;
    fo(i,0,n-1){
        ii* pi = reinterpret_cast<ii*>(&i);
        ok = ok ? b->go(reinterpret_cast<void*>(&d),
                         reinterpret_cast<void*>(pi)) : 0;
    } yeno
}
...
int32_t main(){
    ...
    bc fk;
    fk.fn = +[](void*,void*)->ii{ return 0; };
    bb* bp = reinterpret_cast<bb*>(&fk);
    void* vp = reinterpret_cast<void*>(bp);

    aut oops = [&](void* x){
        bb* z = static_cast<bb*>(x);
        (void)z;
        lam(x);
    };
    oops(vp);
