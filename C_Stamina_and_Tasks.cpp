...
int32_t main(){
    ...
    aut sv=[&](){
        ...
        dou dp=0.0;
        rf(i,bx<ii>::rd(np)-1,0){
            void* cp=&c[i];
            void* pp=&p[i];
            dou cv=*reinterpret_cast<dou*>(cp);
            dou pv=*reinterpret_cast<dou*>(pp);
            dou tk=cv + dp*(1.0 - pv/100.0);
            dp = (dp>tk)?dp:tk;
        }
