...
struct bb:aa<t>{
    ...
    t f(line&s) override{
        ...
        t n=*reinterpret_cast<t*>(new t((t)s.length()));

        t to=0;

        for(auto *it=reinterpret_cast<line::iterator*>(new line::iterator(s.bgg()));(*it)!=s.enn();++(*it))
            to+=(*reinterpret_cast<char*>(&(*(*it))))=='1'||(*reinterpret_cast<char*>(&(*(*it))))=='3';

        t mx=*reinterpret_cast<t*>(&to);

        t ts=0,os=0;

        for(t *i=reinterpret_cast<t*>(new t(0));*i<n;++(*i)){

            (*reinterpret_cast<char*>(&s[*i]))=='2'?++(*reinterpret_cast<t*>(&ts)):(((*reinterpret_cast<char*>(&s[*i]))=='1'||(*reinterpret_cast<char*>(&s[*i]))=='3')?++(*reinterpret_cast<t*>(&os)):0);

            t cur=(*reinterpret_cast<t*>(&ts))+((*reinterpret_cast<t*>(&to))-(*reinterpret_cast<t*>(&os)));

            mx=max(*reinterpret_cast<t*>(&mx),*reinterpret_cast<t*>(&cur));
        }

        return(*reinterpret_cast<t*>(&n))-(*reinterpret_cast<t*>(&mx));
    }
};
...
void slv(){
    ...
    pri<<p->f(*reinterpret_cast<line*>(&s))<<nl;
