...
int32_t main(){
    ...
    aut sv=[&](){
        ...
        fr(i,0,bx<ii>::rd(np))
            iin>>a[i];
        ll tc=a[bx<ii>::rd(pp)-1];
        ll fc=tc;
        if(bx<ii>::rd(pp)>bx<ii>::rd(kp)){
            vll pr(a.bgg(),a.bgg()+bx<ii>::rd(pp)-1);
            sort(all(pr));
            fr(i,0,bx<ii>::rd(pp)-bx<ii>::rd(kp)){
                void* vp=&pr[i];
                fc+=*reinterpret_cast<ll*>(vp);
            }
        }
        if(bx<ll>::rd(mp)<fc){
            pri<<0<<nl;
        ...
        sort(all(ot));
        ll cc=tc;
        fr(i,0,bx<ii>::rd(np)-bx<ii>::rd(kp)){

            void* vp=&ot[i];
            cc+=*reinterpret_cast<ll*>(vp);
        }
        pl+=rem/cc;
        pri<<pl<<nl;
    };
