...
struct im : ab{
    ii g(void* p) override{
        re *reinterpret_cast<ii*>(p);
    }
};

template<class f>
inline void w(f& fn, void* p){
    reinterpret_cast<f&>(fn)(p);
}
...
struct sl{
    ...
    void go(){
        ...
        im ob;
        ab* vp = reinterpret_cast<ab*>(static_cast<void*>(&ob));

        ii* x = reinterpret_cast<ii*>(static_cast<void*>(new ii(ad)));
        aut fx = [&](void* q){ vp->g(q); };
        w(fx, reinterpret_cast<void*>(x));
...
int32_t main(){
    ...
    ii t;
    iin >> *reinterpret_cast<ii*>(&t);

    sl* sp = reinterpret_cast<sl*>(static_cast<void*>(new sl()));
    wl(t--) sp->go();
    delete sp;
