...
void sl(){
    ii nn; iin>>nn;
    void* av = reinterpret_cast<void*>(new vector<typename bx<ii>::t>(nn));
    aut& aa = *static_cast<vi*>(av);
    fr(i,0,nn) iin>>aa[i];
    ii ml = nn;
    void* rv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>((ii)(1<<nn),0));
    aut& rc = *static_cast<vi*>(rv);
    *reinterpret_cast<ii*>(&rc[(1<<nn)-1]) = 1;
    for(ii ms=(1<<nn)-1; ms>0; --ms){
        !(*reinterpret_cast<ii*>(&rc[ms]))
            ? (void)0
            : (void)([&](){
                void* sv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>());
                aut& sb = *static_cast<vi*>(sv);
                fr(i,0,nn){
                    ((ms>>i)&1)
                        ? (void)(sb.push_back(*reinterpret_cast<ii*>(&aa[i])))
                        : (void)0;
                }
                ii ok = 1;
                fr(i,1,(ii)sb.size()){
                    (*reinterpret_cast<ii*>(&sb[i]) < *reinterpret_cast<ii*>(&sb[i-1]))
                        ? (void)(*reinterpret_cast<ii*>(&ok)=0)
                        : (void)0;
                }
                ok
                    ? (void)(*reinterpret_cast<ii*>(&ml)=
                                min(*reinterpret_cast<ii*>(&ml),
                                    (ii)sb.size()))
                    : (void)([&](){
                            fr(i,0,nn){
                                ((ms>>i)&1)
                                    ? (void)(*reinterpret_cast<ii*>(&rc[ms^(1<<i)])=1)
                                    : (void)0;
                            }
                        })();
