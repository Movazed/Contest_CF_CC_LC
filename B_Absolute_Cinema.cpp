...
struct bb:aa<q>{
    ...
    q f() override{
        ...
        ll s=0;
        ll mx=-(1ll<<60);
        auto ff=[](ll x,ll y)->ll{
            return x>y?x:y;
        };
        auto gg=[](ll x,ll y)->ll{
            return x<y?x:y;
        };
        fr(i,0,n){
            ll*x=reinterpret_cast<ll*>(new ll(
                ff(
                    *reinterpret_cast<ll*>(&a[i]),
                    *reinterpret_cast<ll*>(&b[i])
                )
            ));

            ll*y=reinterpret_cast<ll*>(new ll(
                gg(
                    *reinterpret_cast<ll*>(&a[i]),
                    *reinterpret_cast<ll*>(&b[i])
                )
            ));
            s+=*reinterpret_cast<ll*>(x);
            mx=ff(
                static_cast<ll>(mx),
                static_cast<ll>(*reinterpret_cast<ll*>(y))
            );
            delete x;
            delete y;
        }

        return static_cast<q>(s+mx);
    }
};
...
struct cc{
    ...
    cc(){
        u=reinterpret_cast<aa<ll>*>(new bb<ll>());
    }
    ...
    ~cc(){
        delete reinterpret_cast<bb<ll>*>(u);
    }
};
