...
#define wl while
#define sof sizeof
#define fr(v,n) for(ii i=0;i<(ii)(n);i+=1)
#define frk(v,c,n,k) for(ii v=c;v<n;v+=k)
#define pb push_back
...
struct st{
    ...
    ii sz;
    st(ii n):dt(n),sz(n){}
    tp& operator[](ii i){re dt[i];}
    ii szz(){re sz;}
};

template<typename tp,typename kp>
struct mp{
    map<tp,kp> dt;
    kp& operator[](tp k){re dt[k];}
    bool has(tp k){re dt.count(k)>0;}
    auto bgg(){re dt.bgg();}
    auto enn(){re dt.enn();}
};

struct sv{
    function<void(st<ii>&,st<ii>&,ii,ii,bool&,vi&,vi&)> aly;
    function<bool(vi&,vi&)> vrf;
    sv(){
        aly=[](st<ii>&a,st<ii>&b,ii k,ii n,bool&ok,vi&pl,vi&rq){
            frk(c,0,k,1){
                bool mn=true;
                frk(i,c+k,n,k) mn=mn&&(a[i]==a[c]);
                !mn?[&](){
                    frk(i,c,n,k)(b[i]!=-1&&b[i]!=a[i])?(ok=false):0;
                }():[&](){
                    pl.pb(a[c]);
                    ii fv=-1;
                    frk(i,c,n,k){
                        b[i]!=-1?(fv==-1?(fv=b[i]):(fv!=b[i]?(ok=false):0)):0;
                    }
                    fv!=-1?(rq.pb(fv),(void)0):(void)0;
                }();
            }
        };
        vrf=[](vi&pl,vi&rq)->bool{
            mp<ii,ii> ct;
            fr(x,pl.szz()) ct[pl[i]]++;
            bool ok=true;
            fr(x,rq.szz()) ct.has(rq[i])&&ct[rq[i]]>0?ct[rq[i]]--:(ok=false);
            re ok;
        };
    }
};
...
void slv(){
    ...
    vi pl,rq;
    sv s;
    s.aly(a,b,k,n,ok,pl,rq);
    ok=ok?s.vrf(pl,rq):false;
    pri<<(ok?"YES\n":"NO\n");
}
