...
void sl(){
    ...
    struct dd{
        aa* p; aa* q;
        dd():p(new bb<ll>()),q(new cc<ll>()){}
        ~dd(){ delete p; delete q; }

        void ad(void* a,void* b){ p->f(a,b); }
        void ml(void* a,void* b){ q->f(a,b); }
    ...
    ll a0=c0(x-1);
    ll b0=(c0(n)-c0(x-1)+md)%md;

    ll a1=c1(x-1);
    ll b1=(c1(n)-c1(x-1)+md)%md;

    ll z=a0;

    o.ml(reinterpret_cast<void*>(&z),
         reinterpret_cast<void*>(&b0));

    ll y=a1;

    o.ml(reinterpret_cast<void*>(&y),
         reinterpret_cast<void*>(&b1));

    o.ad(reinterpret_cast<void*>(&z),
