...
stc dp:a0{
    void rn(void* pv) override{
        ...
        (iin>>s)?
            (void)([&](){
                auto it=reinterpret_cast<string*>(&s)->bgg();
                wl(it!=reinterpret_cast<line*>(&s)->enn())++it;
                pri<<*reinterpret_cast<line*>(&s)<<"s"<<nl;
            }()):
            (void)0;
    }
};
...
stc sl:rt{
    void go() override{
        ...
        a0* po=reinterpret_cast<a0*>(static_cast<a0*>(&o));
        a1* pm=reinterpret_cast<a1*>(static_cast<a1*>(&m));
        a2* px=reinterpret_cast<a2*>(static_cast<a2*>(&x));
...
int32_t main(){
    ...
    auto f=[&](){ reinterpret_cast<rt*>(rp)->go(); };
    h0<decltype(f)>::g(f);
