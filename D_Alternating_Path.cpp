...
struct it{
    ...
    ii& v(){
        return *(reinterpret_cast<ii*>(p)+i);
    }
};
...
void sl(){
    ...
    void* np=&n;
    void* mp=&m;
    vvi ad(bx<ii>::rd(np)+1);
    fr(i,0,bx<ii>::rd(mp)){
        ii u,v;
        iin>>u>>v;
        void* up=&u;
        void* vp=&v;
        ad[bx<ii>::rd(up)].pb(bx<ii>::rd(vp));
        ad[bx<ii>::rd(vp)].pb(bx<ii>::rd(up));
    }
    vi cl(bx<ii>::rd(np)+1,-1);
    void* cp=&cl[0];
    ii an=0;
    void* ap=&an;
    fr(i,1,bx<ii>::rd(np)+1){
        it c(cp,i);
        (c.v()!=-1)
        ?
        0
