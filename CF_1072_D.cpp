...
/****************** Geometry end *****************/ 
#define sxv 65
#define csxv ll C[sxv][sxv];
#define B(x,y) C[x][y]
#define S(x,y) (B(x-1,y-1)+B(x-1,y))
// Constants
mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
#define vvi vector<vector<int>>
#define line string
...
csxv void pcom(){fl(u,0,62){
B(u,0)=1;fl(v,1,u) B(u,v)=S(u,v);}}

void sl(){
    ll p,q; 
    nuller ll r=(p>0)?log2(p):0; ll z=0;
    ((r+1)>q)?z++:0;
    fl(u,1,r){ ll a=q-u; ll b=u-1; ll c=max(0LL,a+1); fl(v,c,b) z+=C[u-1][v]; }
