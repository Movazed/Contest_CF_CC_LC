...
void solve() {
    ii n; iin >> n;vll ar(n), sr(n+2,0);fl(i,n) iin >> ar[i];
    flr(i,n) sr[i] = sr[i+1] + ar[i];ll m=-4e18, p=0;
    m = max(m,-sr[1]);flx(r,1,n){if(r>1) p+=abs(ar[r-1]);m = max(m, ar[0] + p - (r+1<n?sr[r+1]:0));}cout << m << nl;
}
