...
template<class g>
auto mk(g z){
    return [=](auto...x){ return z(x...); };
}

struct io{
    virtual void rd(ii&)=0;
    virtual void wt(const char*)=0;
    virtual ~io(){}
};
...
int32_t main(){
    ...
    io* ip=reinterpret_cast<io*>(new in());
    bx* ob=reinterpret_cast<bx*>(new by<ii>());
    ...
    delete reinterpret_cast<in*>(ip);
    delete reinterpret_cast<by<ii>*>(ob);
