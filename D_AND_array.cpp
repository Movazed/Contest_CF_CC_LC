...
struct pr{
    ta a;tb b;
    pr(ta x,tb y):a(x),b(y){}
};

struct cm{
    function<ll(ll,ll)> pw;
    function<ll(ll)> inv;
    function<ll(ii,ii)> ncr;
    cm(){
        pw=[](ll bs,ll ep)->ll{
            ll rs=1;bs%=md;
            while(ep>0){
                ep%2==1?(void)(rs=rs*bs%md):(void)0;
                bs=bs*bs%md;ep/=2;
            }
            return rs;
        };
        inv=[&](ll n)->ll{return pw(n,md-2);};
        ncr=[&](ii n,ii r)->ll{
...
struct pc{
    ...
    pc(cm&c){
        run=[&](){
            fc[0]=ifc[0]=1;
            frk(i,1,mx) fc[i]=fc[i-1]*i%md;
            ifc[mx-1]=c.inv(fc[mx-1]);
            frkd(i,mx-2,1) ifc[i]=ifc[i+1]*(i+1)%md;
        };
    }
};
    ...
    sv(cm&c){
        slv=[&](ii n){
            ...
            frkd(k,n,1){
                ll cs=0;
                fr(x,ac.size()){
                    ii cnt=ac[i].a;ll ak=ac[i].b;
                    cs=(cs+ak*c.ncr(cnt,k))%md;
                }
                ll ak=(b[k]-cs)%md;
                ak<0?(void)(ak+=md):(void)0;
                ak!=0?(ac.pb(pr<ii,ll>(k,ak)),(void)0):(void)0;
            }
            ar<ii> a(n,0);
            fr(x,ac.size()){
                ii cnt=ac[i].a;ll mk=ac[i].b;
                frk(j,0,29){
                    (mk>>j)&1?[&](){
                        frk(p,0,cnt) a[p]|=(1<<j);
                    }():(void)0;
