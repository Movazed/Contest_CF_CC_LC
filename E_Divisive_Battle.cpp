...
void pc(){
    fr(i,1,maxa+1) spf[i]=i;
    fr(i,2,(ii)(sqrt(maxa))+1){
        spf[i]==i
            ? (void)([&](){
                    for(ii j=i*i;j<=maxa;j+=i)
                        spf[j]==j ? spf[j]=i : j;
              }())
            : (void)0;
    }
        ...
        : (void)([&](){
            ...
            fr(i,0,nn){
                aa[i]==1
                    ? (void)(pp[i]=1)
                    : (void)([&](){
                        ii pr = spf[*reinterpret_cast<ii*>(&aa[i])];
                        ii tp = *reinterpret_cast<ii*>(&aa[i]);
                        wh(tp%pr==0) tp/=pr;
                        tp>1
                            ? (void)(hm=true)
                            : (void)0;
