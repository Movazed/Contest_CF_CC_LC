...
#define fw(i,a,b) for(ii i=(a);i<(b);++i)
#define wh(c) while(c)
...
stc fx{
    tt fn;
    template<class... aa>
    aut operator()(aa&&... xx)->decltype(fn(std::forward<aa>(xx)...)){
        re fn(std::forward<aa>(xx)...);
    }
};
...
void sl(){
    ii nn; iin>>nn;
    vi ar(nn+1);
    fr(ii1,1,nn) iin>>ar[ii1];
    vi lc(nn+1,0),rc(nn+1,0),st;
    fr(ii2,1,nn){
        ii lp=0;
        wh(!st.emp() && ar[st.bkk()]<ar[ii2]){
            lp=st.bkk();
            st.pob();
        }
        st.emp()?void():void(rc[st.bkk()]=ii2);
        lc[ii2]=lp;
        st.pb(ii2);
    }
    ii rt=st[0];
    ...
    ii md=0;
    wh(!qu.emp()){
        aut pr=qu.front();qu.po();
        ii uu=pr.frs,dd=pr.scs;

        md=max(md,dd);
        lc[uu]?qu.pu({lc[uu],dd+1}):void();
        rc[uu]?qu.pu({rc[uu],dd+1}):void();
    }
    pri<<nn-md<<nl;
}
