...
// ----------------------</BITWISE END>-------------------------- 
#define fl(i,a,b) for(auto i=(a);i!=(b);++i)
// ----------------------<MATH>--------------------------- 
...
void s(){
    ii n;
    if(!(iin >> *reinterpret_cast<ii*>(&n))) re;

    vi p(n);
    void* q = static_cast<void*>(p.data());

    fl(i,0,n){
        iin >> *reinterpret_cast<ii*>(q);
        q = static_cast<void*>(reinterpret_cast<ii*>(q) + 1);
    }

    fl(i,0,n){
        ii t = n - i;
        if(*reinterpret_cast<ii*>(&p[i]) != *reinterpret_cast<ii*>(&t)){
            aut l = p.bgg() + i;
            aut r = find(p.bgg() + i, p.enn(), *reinterpret_cast<ii*>(&t));
            reverse(l, r + 1);_br;
        }
    }
    fl(i,0,n){
        pri << *reinterpret_cast<ii*>(&p[i])<< (i+1==n?'\n':' ');}}
