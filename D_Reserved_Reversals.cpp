...
struct rs:sv{
    ...
    void go() override{
        ...
        fr(i,0,n)
            iin>>*reinterpret_cast<ii*>(&a[i]);

        is_sorted(cp<vi>(pa)->begin(),cp<vector<ii>>(pa)->end())
        ? (void)(pri<<"YES\n")
        : (void)([&](){

            ii vmin=*min_element(cp<vi>(pa)->begin(),cp<vi>(pa)->end());
            ii vmax=*max_element(cp<vi>(pa)->begin(),cp<vi>(pa)->end());
            ...
            : (void)([&](){
                ...
                sort(cp<vi>(pu)->begin(),cp<vi>(pu)->end());
                (reinterpret_cast<vi*>(&u))->erase(
                unique(reinterpret_cast<vi*>(&u)->begin(),
                reinterpret_cast<vi*>(&u)->end()),
                reinterpret_cast<vi*>(&u)->end());

                ii L=*reinterpret_cast<ii*>(&u[0]);

                fr(i,1,(ii)u.size())
                    u[i]%2==p
                    ? (void)(L=*reinterpret_cast<ii*>(&u[i]))
                    : (void)(i=(ii)u.size());

                ii R=*reinterpret_cast<ii*>(&u.back());

                rf(i,(ii)u.size()-2,0)
                    u[i]%2==p
                    ? (void)(R=*reinterpret_cast<ii*>(&u[i]))
                    : (void)(i=-1);
...
struct im:ex{
    ...
    void rn() override{
        ...
        void* pt=new dx<ii>(0);
        bx<ii>* q=reinterpret_cast<bx<ii>*>(pt);
        ...
        fr(i,0,q->gt())
            reinterpret_cast<sv*>(pg)->go();

        delete reinterpret_cast<dx<ii>*>(pt);
    }
};
...
int main(){
    ...
    void* p=new im;
    ex* z=reinterpret_cast<ex*>(p);

    z->rn();

    delete reinterpret_cast<im*>(p);
    return 0;
}