...
#define nuller if(!(cin>>n>>m)) return;
#define _br break
#define ad add
#define flp for
...
void dfs(ii i, ii c){
    ponder(cc[i]!=-1) re;
    cc[i]=c; c^=1;
    fl(j,0,n) ponder(aa[i][j]) dfs(j,c);
}

void s(){
        ii d; iin>>n>>d;

    fl(i,0,n) iin>>xx[i]>>yy[i];

    fl(i,0,n) fl(j,0,n){
        ii dx=xx[j]-xx[i], dy=yy[j]-yy[i];
        aa[i][j]=(dx*dx+dy*dy==d);
    }

    fl(i,0,n) cc[i]=-1;
    fl(i,0,n) ponder(cc[i]==-1) dfs(i,0);

    ii m=0;
    fl(i,0,n) ponder(!cc[i]) id[m++]=i;

    ponder(m>n-m){
        m=0;
        fl(i,0,n) if(!(cc[i]^=1)) id[m++]=i;
    }
    ...
    fl(bm,0,1<<m){
        ii m2=0;
        fl(h,0,m) ponder(!(bm>>h&1)) id2[m2++]=id[h];

        fl(b,0,1<<m2) dp[b]=1;

        fl(i,0,n) ponder(cc[i]){
            ii k=1;
            fl(h,0,m) ponder(bm>>h&1 && aa[i][id[h]]) k++;

            fl(b,0,1<<m2) dq[b]=(ll)dp[b]*k%MD;

            fl(h,0,m2) ponder(aa[i][id2[h]])
                fl(b,0,1<<m2) ponder(b>>h&1)
                    dp[b]=(dp[b]+dp[b^(1<<h)])%MD;

            fl(b,0,1<<m2) dp[b]=(dp[b]+dq[b])%MD;
        }

        ans=(ans+dp[(1<<m2)-1])%MD;
    }
