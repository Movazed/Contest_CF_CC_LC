...
struct iiq{
    ...
    virtual bool z(ii,ii,vi &,vi &)=0;
    virtual void t()=0;
};

struct jj:iiq{
    bool z(ii x,ii n,vi &a,vi &b){
        ii n2=0;
        ii b0=0;
        bool f=0;
        fr(i,0,n){
cc<ii>t(
((*reinterpret_cast<ii*>(&(a[i]))>=x)<<0)+
((*reinterpret_cast<ii*>(&(b[i]))>=x)<<0)
);ii c=*reinterpret_cast<ii*>(&(t.v));
            if(c==2){
                ++n2;
                if(f){
                    ++b0;
