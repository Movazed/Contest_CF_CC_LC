...
struct cd:ab<tt>{
    ...
    cd(function<void(void*)> f):fn(f){}
    void go(void* p){ fn(p); }
};

template<class tt>
ab<tt>* mk(function<void(void*)> f){
    return new cd<tt>(f);
}
...
void sx(){
    ...
    ii mx=0,uc=0,pn=-1;
    aut rd=[&](void* vp){
        aut it=reinterpret_cast<vi*>(vp)->bgg();
        fr(is,0,nn){
            iin>>*(it+is);
            ii v=*(it+is);
            mx=(v>mx)?v:mx;
    ...
    ab<vi>* h1=mk<vi>(rd);
    h1->go(reinterpret_cast<void*>(&pp));

    aut sw=[&](void* vp){
        aut* p=reinterpret_cast<vi*>(vp);
        (uc>1)?(void)iter_swap(p->bgg(),p->bgg()+pn):(void)0;
    };

    ab<vi>* h2=mk<vi>(sw);
    h2->go(reinterpret_cast<void*>(&pp));
    ...
    ab<vi>* h3=mk<vi>(ot);
    h3->go(reinterpret_cast<void*>(&pp));
    delete h1;
    delete h2;
    delete h3;
}
