...
struct rs:sv{
    ...
    auto mn(a x,b y){ return x<y?x:y; }
    ...
    void go() override{
        ...
        fr(i,0,n){
            ...
            iin>>v;
            ts+=v;
            reinterpret_cast<vector<ll>*>(((i+1)%2)?ap:bp)->push_back(v);
        }
        ...
        sort(cp<vll>(ap)->begin(),cp<vector<ll>>(ap)->end(),
        [](ll a,ll b){ return a>b; });

        sort(cp<vll>(bp)->begin(),cp<vector<ll>>(bp)->end(),
        [](ll a,ll b){ return a>b; });

        ll ms=0;

        auto ad=[&](void* p,ii c)->void{
            auto& v=*reinterpret_cast<vector<ll>*>(p);

            v.empty()||c<=0 ? void() :
            (
                ms+=*reinterpret_cast<ll*>(&v[0]),
                [&](){
                    ii lm=mn(c,(ii)v.size());
                    fr(i,1,lm)
                        v[i]>0?ms+=*reinterpret_cast<ll*>(&v[i]):0;
                }()
            );
        };

        ad(reinterpret_cast<void*>(&op),co);
        ad(reinterpret_cast<void*>(&ep),ce);
