...
struct sl{
    void operator()(){
        ...
        vll a(n+1);
        fr(i,1,n) iin>>reinterpret_cast<ll*>(&a[0])[i];
        ...
        fr(c,1,n){
            ll cm=0;
            rf(i,c,1){
                cm=max(cm,reinterpret_cast<ll*>(&a[0])[i]);
                p[c]+=cm;
            }
        }
        ...
        vll c=p;
        vll mx(n+1,0);
        fr(i,1,n) mx[i]=reinterpret_cast<ll*>(&a[0])[i];
        ...
        fr(k,1,n){
            ...
            fr(i,1,k){
                (k>i)?
                (void)(
                    mx[i]=max(mx[i],reinterpret_cast<ll*>(&a[0])[k]),
                    c[i]+=mx[i]
                )
                :
                (void)0;
        ...
        vll cr=pr;
        vll mxr(n+1,0);
        fr(i,1,n) mxr[i]=reinterpret_cast<ll*>(&a[0])[i];
        ...
        rf(k,n,1){
            ...
            rf(i,n,k){
                (k<i)?
                (void)(
                    mxr[i]=max(mxr[i],reinterpret_cast<ll*>(&a[0])[k]),
                    cr[i]+=mxr[i]
                )
                :
                (void)0;
