        ...
        : (used[i][cx][cy] 
            ...
            : ([&]() {
                ...
                for (ll bit = 0; bit < 2; bit++) {
                    ...
                    best = (!(A == 1 && B == 1) 
                        ? min(best, (([&]() {
                            ll sub = solve_dp(i + 1, ncx, ncy);
                            return sub != INF ? (bit << i) + sub : INF;
                        })()))
                        : best);
                }

                return dp[i][cx][cy] = best;
            })());
}

void solve() {
    cin >> X >> Y;
    cout << (X == Y ? -1 : (X & Y) == 0 ? 0 : (memset(used, 0, sizeof(used)), (solve_dp(0, 0, 0) > LIMIT ? -1 : solve_dp(0, 0, 0)))) << "\n";
}
