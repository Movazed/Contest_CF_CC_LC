...
#define wh(x) while(x)
#define rt return
#define tr(c,a,b) ((c)?([&](){a;}()):([&](){b;}()))

#define q1 "? "
#define sp " "
#define nl "\n"
#define out(x) (*(reinterpret_cast<ostream*>(&cout)))<<(x)
#define askp(i,j) out(q1)<<i<<sp<<j<<nl
#define fl (*(reinterpret_cast<ostream*>(&cout))).flush()

template<class t>
struct a0{ virtual t f(void*)=0; virtual ~a0(){} };

template<class t>
struct a1:a0<t>{
    function<t(void*)> g;
    a1(function<t(void*)> _g):g(_g){}
    t f(void* x){ return g(x); }
};

template<class t>
struct a2{ t v; a2(t _v):v(_v){} };

struct a3{ void* p; a3(void* x):p(x){} void* g(){ return p; } };

int main(){
    nfio
    ii t; iin>>t;
    a0<void>* ob = new a1<void>(
        [&](void* x)->void{
            ii n; iin>>n;
            aut ak = [&](ii i, ii j)->ii{
                askp(i,j); fl;
                ii r; iin>>r;
                void* px = static_cast<void*>(&r);
                ii rr = *reinterpret_cast<ii*>(px);
                rt rr;
            };
            tr(ak(1,2)==1,
            {
            ...
            },
            {
                ...
                fr(k,0,n-2){
                    ...
                    tpv.pb(u);
                    tpv.pb(v);
                    ii* tp = reinterpret_cast<ii*>(tpv.data());
                    tr(ak(*(tp+0),*(tp+1))==1,
                    {
                        out("! ")<<*(tp+0)<<nl; fl;
                        throw 0;
