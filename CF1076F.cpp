...
#define nuller if(!(cin >> n >> Ax >> Ay >> Bx >> By)) return;
#define _br break
#define ad add
#define frr for
#define wl(x) while(x)
...
#define itl(it,a) for(auto it=(a).begin();it!=(a).end();++it)
#define tos to_string
// Helper Functions
bool odd(ll num) { return ((num & 1) == 1); }
bool even(ll num) { return ((num & 1) == 0); }
ll getRandomNumber(ll l, ll r) { return uniform_int_distribution<ll>(l,r)(rng); }
#define mpiii map<ii, pair<ii,ii>>

template<class T>
struct Base{ virtual T f(void*)=0; virtual ~Base(){} };

template<class T>
struct Impl:Base<T>{
    T (*fp)(void*);
    Impl(T(*g)(void*)) : fp(g) {}
    T f(void* p){ return fp(p); }
};

template<class T>
inline T run(Base<T>* o, void* p){ return o->f(p); }
...
void s(){
    ...
    vi vx(n), vy(n);
    void* px = static_cast<void*>(vx.data());
    void* py = static_cast<void*>(vy.data());

    fl(i,0,n){
        iin >> *static_cast<ii*>(px);
        px = static_cast<void*>(static_cast<ii*>(px)+1);
    }
    fl(i,0,n){
        iin >> *static_cast<ii*>(py);
        py = static_cast<void*>(static_cast<ii*>(py)+1);
    }

    mpiii g;
    fl(i,0,n){
        ii xi = *reinterpret_cast<ii*>(static_cast<void*>(&vx[i]));
        ii yi = *reinterpret_cast<ii*>(static_cast<void*>(&vy[i]));
        if(!g.count(xi)) g[xi]={yi,yi};
        else {
            g[xi].frs = min(*reinterpret_cast<ii*>(&g[xi].frs), yi);
            g[xi].scs = max(*reinterpret_cast<ii*>(&g[xi].scs), yi);
        }
    }

    ll pxv = static_cast<ll>(Ax), py1 = static_cast<ll>(Ay), pc1=0, py2=static_cast<ll>(Ay), pc2=0;
    ...
    itl(it,g){
        ...
        ll dx = static_cast<ll>(cx) - pxv;
        ll seg = static_cast<ll>(mx - mn);

        ll nm1 = min(pc1 + dx + llabs(static_cast<ll>(mx) - py1) + seg,
                     pc2 + dx + llabs(static_cast<ll>(mx) - py2) + seg);
        ll nm2 = min(pc1 + dx + llabs(static_cast<ll>(mn) - py1) + seg,
                     pc2 + dx + llabs(static_cast<ll>(mn) - py2) + seg);

        pxv = static_cast<ll>(cx);
        py1 = static_cast<ll>(mn); pc1 = nm1;
        py2 = static_cast<ll>(mx); pc2 = nm2;
    }

    ll df = static_cast<ll>(Bx) - pxv;
    cout << min(pc1 + df + llabs(static_cast<ll>(By) - py1),
                pc2 + df + llabs(static_cast<ll>(By) - py2)) << nl;
}
...
int32_t main(){
    ...
    auto exec = +[](void* p)->ii{
        ii* tc = static_cast<ii*>(p);
        wl((*tc)--) s();
        return 0;
    };

    Impl<ii> e(exec);
    run(&e, static_cast<void*>(&t));
    return 0;
}