...
struct im:ab<t>{
    function<t(void*)> f;
    im(function<t(void*)> g):f(g){}
    t go(void* p){return f(p);}
};
...
struct it{
    void* p;
    ii i;
    it(void* a,ii b):p(a),i(b){}
    ll& v(){
        re *(reinterpret_cast<ll*>(p)+i);
    }
};

ll gl(void* ap,void* bp){
    ll a=*reinterpret_cast<ll*>(ap);
    ll b=*reinterpret_cast<ll*>(bp);
    ll g=std::gcd(a,b);
    ll mx=static_cast<ll>(2e18);
    re (a==0||b==0)
?0:((a/g>mx/b)?mx:(a/g)*b);
}
...
void sl(){
    ...
    void* cp=&c;
    void* mp=&m;
    ll va=bx<ll>::rd(mp)/bx<ll>::rd(ap);
    ll vb=bx<ll>::rd(mp)/bx<ll>::rd(bp);
    ll vc=bx<ll>::rd(mp)/bx<ll>::rd(cp);
    ll ab=gl(ap,bp);
    ll ac=gl(ap,cp);
    ll bc=gl(bp,cp);
    void* abp=&ab;
    void* acp=&ac;
    void* bcp=&bc;
    ll vab=bx<ll>::rd(mp)/bx<ll>::rd(abp);
    ll vac=bx<ll>::rd(mp)/bx<ll>::rd(acp);
    ll vbc=bx<ll>::rd(mp)/bx<ll>::rd(bcp);
    ll t=gl(bp,cp);
    void* tp=&t;
    ll abc=gl(ap,tp);
    void* abcp=&abc;
    ll vabc=bx<ll>::rd(mp)/bx<ll>::rd(abcp);
    ll wa=6*va-3*vab-3*vac+2*vabc;
    ll wb=6*vb-3*vab-3*vbc+2*vabc;
    
    ll wc=6*vc-3*vac-3*vbc+2*vabc;
    void* wap=&wa;
    void* wbp=&wb;
    void* wcp=&wc;
    pri<<*reinterpret_cast<ll*>(wap)<<" "<<*reinterpret_cast<ll*>(wbp)<<" "<<*reinterpret_cast<ll*>(wcp)<<nl;
}
int32_t main(){
    nfio
    ll z=0;
    aut l=[&](void* p)->ll{
        re *reinterpret_cast<ll*>(p);
    };
    ab<ll>* x=new im<ll>(l);
    ii t;
    iin>>t;
    void* tp=&t;
    ii* tr=reinterpret_cast<ii*>(tp);
    wh((*tr)--){
        sl();
    }
    x->go(&z);
