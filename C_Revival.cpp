...
#define repd(x,y) for(int x=y;x>=1;x--)
#define repb(x,y,z) for(int x=y;x>=0;x--)
...
struct ft{
    ...
    vi t;
    ft(ii n):n(n),t(n+1,0){}
    function<void(ii,ii)>ad=[&](ii i,ii d){for(;i<=n;i+=i&-i)t[i]+=d;};
    function<ii(ii)>qr=[&](ii i){ii s=0;for(;i>0;i-=i&-i)s+=t[i];return s;};
    function<ii(ii)>fk=[&](ii k)->ii{
        ii idx=0;
        repb(i,N-1,0){ii nx=idx+(1<<i);(*reinterpret_cast<ii*>(&nx)<=n&&t[*reinterpret_cast<ii*>(&nx)]<k)?(idx=*reinterpret_cast<ii*>(&nx),k-=t[idx],0):0;}
        return idx+1;
    };
};
struct sv{
    ...
    void rd(){
        cin>>n;
        p.assign(n+1,-1);s.assign(n+1,-1);c.assign(n+1,0);an.assign(n+1,0);
        rep(i,n){char tp;cin>>tp;tp=='p'?cin>>p[i]:cin>>s[i];}
    }
    void ps1(){
        s[0]=0;ft<>bp(n);ll ci=0;
        rep(i,n){
            p[i]!=-1?(
                c[i]=bp.qr(n)-bp.qr(p[i]),ci+=c[i],s[i]=ci,bp.ad(p[i],1),0
            ):(
                c[i]=(ii)(s[i]-ci),ci=s[i],0
            );
        }
    }
    void ps2(){
        ft<>ba(n);rep(i,n)ba.ad(i,1);
        repd(i,n){
            p[i]!=-1?(
                an[i]=p[i],ba.ad(p[i],-1),0
            ):(
                an[i]=ba.fk(i-c[i]),ba.ad(an[i],-1),0
            );
        }
    }
    void pr(){rep(i,n)cout<<an[i]<<(i==n?"\n":" ");}
};
void solve(){
    sv *o=new sv();
    o->rd();o->ps1();o->ps2();o->pr();
    delete o;
}
ii main(){
    nfio
    ii t;cin>>t;while(t--)solve();
}