...
struct rs:sv{
    ...
    bool ck(ii k, ii n, vi& a){
        ...
        fr(i,0,n)
            (*reinterpret_cast<ii*>(&a[i])<k && !ut[a[i]])
            ? (void)(ut[a[i]]=1, ue[i]=1)
            : (void)0;
        ...
        rf(i,n-1,0)
            !ue[i]
            ? (void)rc.pb(
                *reinterpret_cast<ii*>(&a[i])==0
                ? -1
                : (*reinterpret_cast<ii*>(&a[i])-1)/2
              )
            : (void)0;
        ...
        fr(i,0,(ii)rt.size())
            if(*reinterpret_cast<ii*>(&rc[i])<
               *reinterpret_cast<ii*>(&rt[i])){
                delete cp<vbo>(pu);
                delete cp<vbo>(pe);
                delete cp<vi>(pr);
                delete cp<vi>(pc);
    ...
    void go() override{
        ...
        sort(cp<vi>(pa)->begin(),cp<vi>(pa)->end());
