...
struct bb:aa<t>{
    ...
    void f() override{
        ...
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<n;++(*i))
            iin>>(*reinterpret_cast<t*>(&a[*i]));
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<n-1;++(*i))
            if((*reinterpret_cast<t*>(&a[*i]))>(*reinterpret_cast<t*>(&a[(*i)+1])))
                d.pb(*reinterpret_cast<t*>(reinterpret_cast<void*>(i)));
        if(d.emp()){
            pri<<"YES\n";
            re;
        }
        ...
        frr(it=d.bgg();it!=d.enn();++it)
            l=max(l,(ll)a[*it]-a[(*it)+1]);
        ll r=(1ll<<62);
        frr(size_t *j=reinterpret_cast<size_t*>(new size_t(0));(*j)+1<d.size();++(*j)){
            t x=d[*j];
            t y=d[(*j)+1];
            ll z=-1;
            frr(t *m=reinterpret_cast<t*>(new t(x+1));*m<y;++(*m))
                z=max(z,(ll)a[(*m)+1]-a[*m]);
            r=min(r,z);
        }
        pri<<(l<=r?"YES\n":"NO\n");
    }
