...
void sl(){
    ...
    dp[1]=(*(a.data())!=*(b.data())?1:0);

    lq(i,2,n){

        ii cv=(*(a.data()+i-1)!=*(b.data()+i-1)?1:0);

        ii o1=dp[i-1];

        o.ad(reinterpret_cast<void*>(&o1),
             reinterpret_cast<void*>(&cv));

        ii ct=(*(a.data()+i-2)!=*(a.data()+i-1)?1:0);
        ii cb=(*(b.data()+i-2)!=*(b.data()+i-1)?1:0);

        ii o2=dp[i-2];

        o.ad(reinterpret_cast<void*>(&o2),
        ...
        o.mn(reinterpret_cast<void*>(&(dp[i])),
             reinterpret_cast<void*>(&o2));
    }
