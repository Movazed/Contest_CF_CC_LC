...
struct iiq:hh{
    ...
    void t(){
        ...
        vi mx(n+1);
        gg g;
auto dfs=[&](auto&&s,ii u,ii p)->void{
    frr(
        auto it=
        reinterpret_cast<vi*>(&ad[u])->bgg();

        it!=reinterpret_cast<vi*>(&ad[u])->enn();

        ++it
    ){
        ii&v=
        *reinterpret_cast<ii*>(&(*it));
        if(
            *reinterpret_cast<ii*>(&(v))
            !=
            *reinterpret_cast<ii*>(&(p))
        ){
            s(
                s,
                *reinterpret_cast<ii*>(&(v)),
                *reinterpret_cast<ii*>(&(u))
            );
        ...
        fr(u,m0+1,n+1){
            ...
            if(lb<u-1){
                ...
                ll z=
                (
                    *reinterpret_cast<ll*>(&(sp[u-1]))
                )-
                (
                    *reinterpret_cast<ll*>(&(sp[lb]))
                );
