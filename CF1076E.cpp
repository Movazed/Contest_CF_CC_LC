...
void sl() {
    ...
    frr (ii i = 0; i < n; ++i) {
        cin >> *static_cast<ii*>(p);
        ii v = *static_cast<ii*>(p);
        if (v == 1) h1 = true;
        else if (v <= n && !seen[v]) {
            seen[v] = 1;
            d.push_back(v);
        }
        p = static_cast<void*>(static_cast<ii*>(p) + 1);
    }
    ...
    if (h1)
        run(&obj, static_cast<void*>(static_cast<ii*>(dpv) + 1));

    frr (aut it = d.bgg(); it != d.enn(); ++it)
        run(&obj, static_cast<void*>(static_cast<ii*>(dpv) + *it));

    frr (ii i = 1; i <= n; ++i) {
        ii cur = *(static_cast<ii*>(dpv) + i);
        if (cur == infy) continue;

        frr (aut it = d.bgg(); it != d.enn(); ++it) {
            ll nx = 1LL * i * (*it);
            if (nx > n) _br;
            ii* slot = static_cast<ii*>(dpv) + nx;
            if (cur + 1 < *slot)
                *slot = cur + 1;
        }
    }
