    ...
    fr(_t,1,tt){
        ...
        aut lm=[&](void*){
            ...
            : ([&](){
                    ...
                    (m==1)
                    ? ([&](){
                            an.pb(cv);an.pb(cv);cv++;
                      }()): (m==2)?([&](){an.pb(cv);an.pb(cv+1);an.pb(cv);
                                an.pb(cv+1);cv=cv+2;}())
                        : ([&](){an.pb(cv);an.pb(cv+1);
                            fr(i,3,m){ii* p1=static_cast<ii*>(reinterpret_cast<void*>(&cv));an.pb((*p1)+i-1);
                                an.pb((*p1)+i-3);}an.pb(cv+m-2);
                                an.pb(cv+m-1);cv+=m;
                          }());
                    fr(i,1,b-1){ ii* pp=&cv;an.pb(*pp);an.pb(*pp);cv++;
                    }
                    fr(i,0,2*n-1){
                        ii* px=&an[i];
                        pri<<*px<<(i==2*n-1?"":" ");
                    }pri<<nl;
              }()); void* vp=reinterpret_cast<void*>(&k);   bx<ii>::cv(vp);
        };
        ab<ii>* ob=new im<ii>(lm);
        void* px=reinterpret_cast<void*>(&n);
        ob->go(px);
        delete ob;}}
struct oox{
    void run(){
