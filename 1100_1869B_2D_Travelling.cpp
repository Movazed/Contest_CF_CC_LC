...
void solve() {
 ...
 ll n, k, a, b, price, pricea_major = LLONG_MAX/2, priceb_minor = LLONG_MAX/2;
 cin >> n >> k >> a >> b;
...
for(ll maj = 1; maj <= k ; maj++){
    ...
    priceb_minor = min(priceb_minor, abs(x[b] - x[maj]) + abs(y[b] - y[maj]));
}

cout << min(price, pricea_major + priceb_minor) << nl;
