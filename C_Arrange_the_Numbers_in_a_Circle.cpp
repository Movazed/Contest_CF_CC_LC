...
template<class t>
stc bb:aa<t>{
    t n,bc,cp,mc,sc,om,c;
    bb():n(0),bc(0),cp(0),mc(0),sc(0),om(0),c(0){}
    t f() override{
        iin>>n;
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<n;++(*i)){
            iin>>(*reinterpret_cast<t*>(&c));
            (*reinterpret_cast<t*>(&c))>=2
            ?
            (
                ++(*reinterpret_cast<t*>(&mc)),
                (*reinterpret_cast<t*>(&bc))+=(*reinterpret_cast<t*>(&c)),
                (*reinterpret_cast<t*>(&cp))+=((*reinterpret_cast<t*>(&c))-2)/2,
                (*reinterpret_cast<t*>(&om))=(*reinterpret_cast<t*>(&c))
            )
            :
            ++(*reinterpret_cast<t*>(&sc));
        }
        if(!(*reinterpret_cast<t*>(&mc)))
            return 0;
        (*reinterpret_cast<t*>(&mc))==1
        ?
        (*reinterpret_cast<t*>(&cp))=(*reinterpret_cast<t*>(&om))/2
        :
        0;
        t mx=(*reinterpret_cast<t*>(&bc))
        +
        min(
            (*reinterpret_cast<t*>(&sc)),
            (*reinterpret_cast<t*>(&cp))

        );

        return mx<3?0:*reinterpret_cast<t*>(&mx);
    }
};
...
void slv(){
    aa<ll>*p=reinterpret_cast<aa<ll>*>(new bb<ll>);
    pri<<p->f()<<nl;
    delete reinterpret_cast<aa<ll>*>(p);
}
