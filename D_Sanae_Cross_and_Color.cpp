...
struct bb : aa<t>
{
    t f(t x) override { return x; }
};

template <class t>
t zz(t x) { return x; }

static auto yy = [](auto &&x) -> auto
{ return forward<decltype(x)>(x); };

template <class t>
static inline t qp(t x, t y, t z) { return x == y ? z : x; }
...
struct cc
{
    ...
    void in()
    {
        ...
        fr(i, 0, n)
        {
            iin >>(*reinterpret_cast<ii *>(&p[i].x))>>(*reinterpret_cast<ii *>(&p[i].y));
            u.pb(*reinterpret_cast<ii *>( &p[i].y));}}

    void mk()
    {
        sort(reinterpret_cast<vi &>(u).bgg(),reinterpret_cast<vi &>(u).enn());
        u.erase(unique(reinterpret_cast<vi &>(u).bgg(),reinterpret_cast<vi &>(u).enn()),reinterpret_cast<vi &>(u).enn());
        sort(reinterpret_cast<vector<pt> &>(p).begin(),reinterpret_cast<vector<pt> &>(p).end(),
            [](const pt &a, const pt &b)
            {
                return
                    *reinterpret_cast<const ii *>(&a.x)
                    <*reinterpret_cast<const ii *>(&b.x);});
        ii cx = *reinterpret_cast<ii *>(&p[0].x);
        ii mn = *reinterpret_cast<ii *>(&p[0].y);
        ii mx = *reinterpret_cast<ii *>(
            &p[0].y);
        fr(i, 1, n)
        {
            if (*reinterpret_cast<ii *>(&p[i].x) == cx)
            {
                mn = min(mn,*reinterpret_cast<ii *>(&p[i].y));
                mx = max(mx,*reinterpret_cast<ii *>(&p[i].y));}
            else
            {
                gp *tp = reinterpret_cast<gp *>(
                    new gp{cx,mn,mx});
                g.pb(
                    *reinterpret_cast<gp *>(tp)
                );
                delete reinterpret_cast<void *>(tp);
                cx = *reinterpret_cast<ii *>(
                    &p[i].x

                );
                mn = *reinterpret_cast<ii *>(
                    &p[i].y

                );
                mx = *reinterpret_cast<ii *>(
                    &p[i].y
                );
            }
        }
        ...
        smn.rss(m);
        smx.rss(m);
        pmn[0] = *reinterpret_cast<ii *>(
            &g[0].a

        );
        pmx[0] = *reinterpret_cast<ii *>(
            &g[0].b
        ...
        fr(i, 0, m - 1)
        {
            ...
            if (l <= r)
            {
                ...
                aut it1 = lower_bound(
                    ...
                    reinterpret_cast<vi &>(u).bgg(),

                    reinterpret_cast<vi &>(u).enn(),
...
void solve()
{
    ...
    aa<ll> *p = reinterpret_cast<aa<ll> *>(
    ...
    delete reinterpret_cast<void *>(u);
}
...
int main()
{
    dd *x = reinterpret_cast<dd *>(
        new ee

    );
    x->go();
    delete reinterpret_cast<void *>(x);
