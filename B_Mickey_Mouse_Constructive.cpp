...
template<class t> struct by:bx<t>{
...
int32_t main(){
    ...
    pc();
    ii t; iin>>t;
    struct op { virtual void run(slv&s)=0; };
    struct qp:op {
        void run(slv&s) override {
            auto l=[&](){ s.go(); };
            l();
        }
    } q;
