...
#define pll_ pair<ll,ll>
#define fr(i,n) for(ii i=0;i<(n);++i)
#define frx(i,a,b) for(ii i=(a);i<(b);++i)
#define frd(i,a,b) for(ii i=(a);i>=(b);--i)
#define fri(it,c) for(auto it=(c).begin();it!=(c).end();++it)

template<class t>
stc tb{
    template<class u> static u ap(u a,u b){return a+b;}
    template<class u> static u mn(u a,u b){return a<b?a:b;}
    template<class u> static u mx(u a,u b){return a>b?a:b;}
};
template<class f>
stc fx{
    f fn;
    template<class... a>
    aut operator()(a&&...x)->decltype(fn(std::forward<a>(x)...)){
        return fn(std::forward<a>(x)...);
    }
};
stc ba{
    virtual void rn(void*)=0;
    virtual ~ba(){}
};
stc sv:ba{
    void sl(){
        ...
        ii to=0;
        fr(i,n)fr(j,m){
            ii* p=reinterpret_cast<ii*>(&aa[i][j]);
            cin>>*p;
            to=tb<ii>::ap(to,*p);
        }
        vvi cp(m,vi(n+1,0));
        fr(j,m)fr(i,n){
            ii* p1=reinterpret_cast<ii*>(&cp[j][i+1]);
            ii* p2=reinterpret_cast<ii*>(&cp[j][i]);
            ii* p3=reinterpret_cast<ii*>(&aa[i][j]);
            *p1=tb<ii>::ap(*p2,*p3);
        }
        vvi mn(m,vi(n+1));
        vvi mx(m,vi(n+1));
        fr(i,n+1){
            ii* p1=reinterpret_cast<ii*>(&mn[0][i]);
            ii* p2=reinterpret_cast<ii*>(&mx[0][i]);
            ii* p3=reinterpret_cast<ii*>(&cp[0][i]);
            *p1=*p3;*p2=*p3;
        }
        frx(j,1,m){
            ii pm=1000000000,px=-1;
            fr(i,n+1){
                pm=tb<ii>::mn(pm,mn[j-1][i]);
                px=tb<ii>::mx(px,mx[j-1][i]);
                ii* p1=reinterpret_cast<ii*>(&mn[j][i]);
                ii* p2=reinterpret_cast<ii*>(&mx[j][i]);
                ii* p3=reinterpret_cast<ii*>(&cp[j][i]);
                *p1=tb<ii>::ap(*p3,pm);
                *p2=tb<ii>::ap(*p3,px);
            }
        }
        ll mp=-1;ii bs=-1,bl=-1;
        aut pr=fx<function<void(ii)>>{[&](ii i){
            ii l=mn[m-1][i],r=mx[m-1][i];
            ii tg=to/2;
            ii cd=tg<l?l:(tg>r?r:tg);
            ll cv=static_cast<ll>(cd)*(to-cd);
            if(cv>mp){mp=cv;bs=cd;bl=i;}
            if(cd+1<=r){
                ll av=static_cast<ll>(cd+1)*(to-(cd+1));
                if(av>mp){mp=av;bs=cd+1;bl=i;}
            }
        }};
      fr(i,n+1)pr(i);
        vi ri(m);
        ii* rp=reinterpret_cast<ii*>(&ri[m-1]);
        *rp=bl;
        ii cv=bs;
        frd(j,m-1,1){
            ii rc=ri[j];
            ...
            ii nd=cv-vl;
            frd(k,rc,0){
                    if(nd >= mn[j-1][k] && nd <= mx[j-1][k]){
                        ii* p_ri = reinterpret_cast<ii*>(&ri[j-1]);
                        ii* p_cv = reinterpret_cast<ii*>(&cv);
                        ii tmp_k = k;
                        ii tmp_nd = nd;
                        *p_ri = *reinterpret_cast<ii*>(&tmp_k);
                        *p_cv = *reinterpret_cast<ii*>(&tmp_nd);
                        _br;
                    }
            }
        }
        ...
        fr(j,m){
            ii tr=ri[j];
            wl(cr<tr){
                pa.pb(*reinterpret_cast<word*>(&line("D")[0]));
                cr++;
            }
            pa.pb(*reinterpret_cast<word*>(&line("R")[0]));
        }
        wl(cr<n){
            pa.pb(*reinterpret_cast<word*>(&line("D")[0]));
            cr++;
    ...
    void rn(void* p){
        aut f=reinterpret_cast<void(*)(void*)>(p);
        f(this);
    }
};
...
int32_t main(){
    nfio
    ii t;cin>>t;
    ba* bp=reinterpret_cast<ba*>(new sv());
    void* fd=reinterpret_cast<void*>(&sv::th);
    fr(i,t){
        bp->rn(fd);
    }
    delete static_cast<sv*>(reinterpret_cast<void*>(bp));
}