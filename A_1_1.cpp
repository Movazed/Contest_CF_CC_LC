...
struct sl{
    void operator()(){
        ...
        auto sf=[&](ii i){
            word *pf=reinterpret_cast<word*>(&fx[i]);
            (*pf)?
            void():
            (
                *pf=1,
                (reinterpret_cast<word*>(&s[0])[i]=='0'?
                    (reinterpret_cast<queue<ii>*>(&qq)->push(i),void()):
                    void())
            );
        };
        ...
        fr(i,0,n-1)
            (reinterpret_cast<word*>(&s[0])[i]=='0' &&
             reinterpret_cast<word*>(&s[0])[i+1]=='0')
            ? (sf(i),sf(i+1))
            : void();

        wh(!reinterpret_cast<queue<ii>*>(&qq)->empty()){
            ii i=reinterpret_cast<queue<ii>*>(&qq)->front();
            reinterpret_cast<queue<ii>*>(&qq)->pop();
        ...
        fr(i,0,n){
            char *pf=reinterpret_cast<char*>(&fx[i]);

            (*pf)?
            (
                (reinterpret_cast<char*>(&s[0])[i]=='1'?(mn++,mx++):0),
                (cm>0?(mx+=cm,mn+=cm/2,cm=0):0)
            )
            :
            cm++;
        }

        (cm>0?(mx+=cm,mn+=cm/2):0);
