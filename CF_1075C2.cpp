...
#define nuller if(n%2==0){ cout<<"-1"<<nl; re; }
#define _br break
#define ad add
#define frr for
...
#define mivii map<int,vector<int>> 
// -------------------------<RNG>------------------------- 
...
/****************** Prime Generator End **********************/ 
#define collapse fl(i,0,k){ii s=(i-1+k)%k;p[a[i]]   = b[s].frs;p[a[i]+1] = b[s].scs;}
#define shorty i^(i+1)
/****************** Geometry *****************/ 
...
#define spp fl(i,1,n+1) pri<<p[i]<<(i==n?"":" ");
#define szz size

stc V{ virtual ii f(ii x){ re x; } };
stc U:V{ ii f(ii x){ re x*2; } };

void s(){
 ii n; iin >> n; nuller

 vi pv(n + 1); void* vp = (void*)&pv;
 vi&p = *static_cast<vi*>(vp);
 p[n] = n;
    mivii g;
    fl(i,1,n) if(i&1) g[shorty].pb(i);

    V* v = new U();
    frr(aut& e:g){
        void* vp2=(void*)&e.scs;
        vi& a = *static_cast<vi*>(vp2);

        vpii b;
        frr(ii x:a) b.pb({x+1,x});
