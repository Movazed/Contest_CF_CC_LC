...
void sl(){
    ii n; iin n;
    line s; iin s;des ok=1;
    fl(i,0,n-1) (s[i]>s[i+1] ? ok=0 : 0);
    if(ok){ pri "Bob\n"; re; }ii z=0;
    flp(word c:s) (c=='0' && (++z));vi mv;
    fl(i,0,z) (s[i]=='1' && (mv.pb(i+1),1));
    fl(i,z,n) (s[i]=='0' && (mv.pb(i+1),1));
    pri "Alice\n"<<mv.ssz;flp(ii x:mv) pri " "<<x;pri nl;
}
