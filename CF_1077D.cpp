...
#define flr(i,a,b) for(ii i=a;i<b;i++)
#define con continue;

stt ll dg;
stt ii dr;

stc ab0{ virtual void rn(void*)=0; virtual ~ab0(){} };
stc ab1{ virtual void rn(ab0*, void*)=0; virtual ~ab1(){} };
stc ab2{ virtual void rn(ab1*, ab0*, void*)=0; virtual ~ab2(){} };
stc ab3{ virtual void in(ii i,ll B,ll X,ll Y,ll a,ll b,ll u,ll v,ll*)=0; virtual ~ab3(){} };

stc dp_in:ab3{
    void in(ii i,ll B,ll X,ll Y,ll a,ll b,ll u,ll v,ll*) override {
        ii na = a==1?(u<X?0:(u>X?2:1)):a;
        ii nb = b==1?(v<Y?0:(v>Y?2:1)):b;
        ll w=0;
        if(na==0) w+=(X-u)*B; else if(na==2) w+=(u-X)*B;
        if(nb==0) w+=(Y-v)*B; else if(nb==2) w+=(v-Y)*B;
        if(d[i+1][a][b]+w<d[i][na][nb]){
            d[i][na][nb]=d[i+1][a][b]+w;
            r[i][na][nb]=(a*3+b)*4+u*2+v;
        }
    }
};
...
stc dp:ab0{
    void rn(void* pv) override {
        ll* xy = reinterpret_cast<ll*>(pv);
        ll xx=xy[0],yy=xy[1];
        fl(j,3) fl(k,3) d[31][j][k]=4e18;
        d[31][1][1]=0;
        stt dp_in in_obj;
        aut ci=[&](ii i,ll B,ll X,ll Y,ll a,ll b,ll u,ll v){ in_obj.in(i,B,X,Y,a,b,u,v,reinterpret_cast<ll*>(d)); };
        for(ii i=30;i>=0;i--){
            fl(j,3) fl(k,3) d[i][j][k]=4e18;

            ll B=1LL<<i;

            ii X=(xx>>i)&1,Y=(yy>>i)&1;

            fl(a,3) fl(b,3){
                if(d[i+1][a][b]>3e18) con
                stt ii us[3][2]={{0,0},{0,1},{1,0}};
                fl(z,3){ ii u=us[z][0],v=us[z][1]; ci(i,B,X,Y,a,b,u,v); }
            }
        }
    }
};

stc md:ab1{ void rn(ab0* o,void* pv) override { o->rn(pv); } };
stc mx:ab2{ void rn(ab1* m,ab0* o,void* pv) override { m->rn(o,pv); } };

stc out{
    static void em(){
        ii a=0,b=0; ll m=4e18;
        fl(j,3) fl(k,3) if(d[0][j][k]<m){ m=d[0][j][k]; a=j; b=k; }
        ll p=0,q=0;
        fl(i,32){
            ii v=r[i][a][b],u=v&3;
            if(u&2) p|=1LL<<i;
            if(u&1) q|=1LL<<i;
            ii pr=v>>2; a=pr/3; b=pr%3;
        }
        pri<<p<<" "<<q<<nl;
    }
};
...
stc sl:rt{
    void go() override {
        static ll xy[2]; cin>>xy[0]>>xy[1];
        static dp o; static md m; static mx k;
        ab0* po=reinterpret_cast<ab0*>(&o);
        ab1* pm=reinterpret_cast<ab1*>(&m);
        ab2* pk=reinterpret_cast<ab2*>(&k);
        pk->rn(pm,po,reinterpret_cast<void*>(xy));
        out::em();
    }
};
...
int32_t main(){
    nfio
    stt sl s;
    rt* r=reinterpret_cast<rt*>(&s);
    function<void()> f=[&](){ r->go(); };
    ii t; cin>>t;
    fl(_,t) f();
}