...
template<class t>
static inline t qp(t x,t y,t z){
    return x==y?z:x;
}
...
struct cc{
    ...
    void mk(){
        word x=static_cast<word>(
            *reinterpret_cast<ii*>(&d[0])+'0'
        );
        word y=static_cast<word>(
            *reinterpret_cast<ii*>(&d[1])+'0'
        );
        line tm=to_string(a);
        line s=*reinterpret_cast<line*>(&tm);
        ii l=static_cast<ii>(s.size());
        if(l>1){
            if(y>'0'){
                line *tp=reinterpret_cast<line*>(
                    new line(line(l-1,y))
                );
                c.pb(*tp);
                delete tp;
        ...
        fr(i,0,l){
            line ts=s.substr(ii(0),i);
            line p=*reinterpret_cast<line*>(&ts);
            if(!(i==0&&x=='0'&&l>1)){
                if(x<s[i]){
                    line *tp=reinterpret_cast<line*>(
                        new line(
            ...
            if(
                *reinterpret_cast<char*>(&s[i])!=x
                &&
                static_cast<char>(s[i])!=y
            )
                _br;
            if(i==l-1)

                c.pb(
                    *reinterpret_cast<line*>(&s)
                );
        }
    }
    ll rt(){
        mk();
        fr(i,0,(ii)c.size()){
            line &s=*reinterpret_cast<line*>(
                &(*next(c.bgg(),i))
            );
            if(static_cast<ii>(s.emp()))
                continue;
            ll v=stoll(
                *reinterpret_cast<line*>(&s)
            );
            ...
            if(mn==static_cast<ll>(-1)||df<mn)
                mn=df;
        }
        re *reinterpret_cast<ll*>(&mn);
    }
};
