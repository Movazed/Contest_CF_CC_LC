...
struct rs:sv{
    ...
    auto mx(a x,b y){ return x<y?y:x; }
    ...
    void go() override{
        ...
        sort(cp<vi>(ps)->bgg(),cp<vi>(ps)->enn());
        ...
        fr(i,0,n){
            *reinterpret_cast<ii*>(&v1[i]) = *reinterpret_cast<ii*>(&a[i]) >= m ? 1 : -1;
            *reinterpret_cast<ii*>(&v2[i]) = *reinterpret_cast<ii*>(&a[i]) <= m ? 1 : -1;
        }

        void* pd=new vi(n+1,-1);
        auto& dp=cv<vi>(pd);
        ...
        fr(i,1,n+1){
            ...
            rf(j,i-1,0){
                s1+=*reinterpret_cast<ii*>(&v1[j]);
                s2+=*reinterpret_cast<ii*>(&v2[j]);

                ((i-j)&1)&&s1>0&&s2>0&&dp[j]!=-1
                ? dp[i]=mx(dp[i],dp[j]+1)
                : 0;
            }
        }
