...
#define fri(it,a) for(auto it=(a).begin();it!=(a).end();++it)

template<class t>
stc ab{
    template<class u>
    static u ix(u a,u b){return a&b;}
};
...
stc sv:bs{
    ...
    void core(){
        ...
        fr(i,k){
            line t; iin>>t;
            fr(j,n){
                cm[j]|=(1<<static_cast<ii>(t[j]-'a'));
            }
        }

        vi dv;
        frx(i,1,static_cast<ii>(sqrt(static_cast<double>(n)))+1){
            if(n%i==0){
                dv.pb(i);
                if(i*i!=n) dv.pb(n/i);
            }
        ...
        aut pick=fx<function<word(ii)>>{[&](ii m)->word{
            ii c=0;
            wl(!((m>>c)&1)) c++;
            word val=static_cast<word>('a'+c);
            return *reinterpret_cast<word*>(&val);
        }};
        ...
        fri(di,dv){
            ...
            fr(r,d){
                ii cmn=(1<<26)-1;
                for(ii j=r;j<n;j+=d)
                    cmn=ab<ii>::ix(cmn,cm[j]);
                if(cmn==0){ ok=false; goto nxt; }
                p+=pick(cmn);
            }

            {
                line a="";
                fr(i,n){
                    word* q=reinterpret_cast<word*>(&p[i%d]);
                    a+=*q;
                }
                pri<<a<<nl;
            }
...
int32_t main(){
    ...
    delete static_cast<sv*>(reinterpret_cast<void*>(ob));
}