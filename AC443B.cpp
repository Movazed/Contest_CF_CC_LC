...
stc dp : a0{
    void rn(void* pv) override{
        ...
        (void)([&](){
            ...
            ll lo=0,hi=2000000000LL,an=hi;

            wl(lo<=hi){
                ll md = lo + (hi-lo)/2;

                unsigned __int128 ct =
                    static_cast<unsigned __int128>(md) +
                    static_cast<unsigned __int128>(1);

                unsigned __int128 sm =
                    static_cast<unsigned __int128>(n) +
                    static_cast<unsigned __int128>(n+md);

                unsigned __int128 tt = (ct*sm)/2;

                (tt >= static_cast<unsigned __int128>(k)) ?
                    (void)(an=md,hi=md-1) :
                    (void)(lo=md+1);
            }
