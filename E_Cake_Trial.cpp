...
ii dp[505][505],nd[505][505];
template<typename fn>
void ex(bool cd,fn&&f){cd?f():void();}
struct sv{
ii n;line s;
sv(){iin>>n>>s;}
ll run(){
fr(i,0,n)fr(j,0,i)dp[j][i]=inf;
dp[0][0]=0;
fr(k,0,n-1){
fr(i,0,k+1)fr(j,0,i)nd[j][i]=inf;
fr(b,0,k)fr(a,0,b){
if(dp[a][b]==inf)continue;
ex(s[k]=='T'||s[k]=='N',[&]{ii na=max(0,a-1),nb=b;*reinterpret_cast<ii*>(&nd[na][nb])=min(*reinterpret_cast<ii*>(&nd[na][nb]),*reinterpret_cast<const ii*>(&dp[a][b])+1);});
ex(s[k]=='F'||s[k]=='N',[&]{ii na=a+1,nb=max(b,na);*reinterpret_cast<ii*>(&nd[na][nb])=min(*reinterpret_cast<ii*>(&nd[na][nb]),*reinterpret_cast<const ii*>(&dp[a][b])-1);});
}
fr(b,0,k+1)fr(a,0,b)dp[a][b]=nd[a][b];
}
ii mv=inf;
fr(b,0,n)fr(a,0,b)dp[a][b]!=inf?mv=min(mv,dp[a][b]+2*b):0;
return(n-mv)/2;
}
};
int main(){
