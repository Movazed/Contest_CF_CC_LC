...
stc ac:ab<tt>{
    void go(void* vp) override{
        aut lm = [&](void* qp){
            aut* tp = reinterpret_cast<
            tuple<ii*,vi*,vi*,vi*,vi*>*>(qp);
            ii* n  = get<0>(*tp);
            aut* l = get<1>(*tp);
            aut* r = get<2>(*tp);
            aut* cs = get<3>(*tp);
            aut* an = get<4>(*tp);
            function<void(ii)> df1 = [&](ii u){
                (((*l)[u]==0)&&((*r)[u]==0))?
                (
                    (*cs)[u]=1,
                    0
                ...
                (
                    df1((*l)[u]),
                    df1((*r)[u]),
                    (*cs)[u]=(((*cs)[(*l)[u]]+(*cs)[(*r)[u]]+3)%md),
                    0
                );
            };
            function<void(ii,ii)> df2 = [&](ii u,ii s){
                (*an)[u]=(s+(*cs)[u])%md,
                ((*l)[u]!=0)?
                    (df2((*l)[u],(*an)[u]),0):0,
                ((*r)[u]!=0)?
...
stc sv:so{
    void run() override{
        ii n;
        iin>>n;
        vi l(n+1),r(n+1),cs(n+1),an(n+1);
        fr(i,1,n) iin>>l[i]>>r[i];
        aut tp = make_tuple(&n,&l,&r,&cs,&an);
        ab<ii>* ob = static_cast<ab<ii>*>(new ac<ii>());
        function<void(void*,ab<ii>*)> fn =
            [](void* p,ab<ii>* o){ o->go(p); };
