...
void sl(){
    ii nn; iin>>nn;
    void* tv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>(nn));
    aut& tt = *static_cast<vi*>(tv);
    fr(i,0,nn) iin>>tt[i];
    void* dpv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>(nn+1,0));
    aut& dp = *static_cast<vi*>(dpv);
    dp[0]=static_cast<ii>(1);
    fr(j,0,nn){
        dp[j]==0 ? (void)0 : (void)([&](){
            ii llx = nn - j;
            void* piv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>(llx,0));
            aut& pi = *static_cast<vi*>(piv);
            ii ln = 0;
            fr(k,1,llx){
                wh(ln>0 && tt[j+k]!=tt[j+ln]) ln = pi[ln-1];
                (*(reinterpret_cast<ii*>(&tt[j+k])) == *(reinterpret_cast<ii*>(&tt[j+ln]))
                    ? ++(*reinterpret_cast<ii*>(&ln))
                    : (*reinterpret_cast<ii*>(&ln)));

                *reinterpret_cast<ii*>(&pi[k]) = *reinterpret_cast<ii*>(&ln);
            }
            fr(k,0,llx){
                pi[k]==0 ?
                    *reinterpret_cast<ii*>(&dp[j+k+1]) = ( *reinterpret_cast<ii*>(&dp[j+k+1]) + *reinterpret_cast<ii*>(&dp[j]) ) % mod
                    : dp[j+k+1];
            }
            delete static_cast<vi*>(piv);

        })();
    }
    pri<<dp[nn]<<nl;
    delete static_cast<vi*>(tv);
    delete static_cast<vi*>(dpv);
}
