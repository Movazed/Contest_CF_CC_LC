...
struct sg{
    ...
    sg(ii n):n(n),tr(4*n+4,0),lz(4*n+4,0){}
    ...
    void ad(ii nd,ii st,ii en,ii l,ii r,ii v){

        (r<st||en<l)
        ? (void)0
        :
        (l<=st&&en<=r)
        ? (void)(tr[nd]+=v,lz[nd]+=v)
        :
        (void)(
            ps(nd),
            ad(2*nd,st,(st+en)/2,l,r,v),
            ad(2*nd+1,(st+en)/2+1,en,l,r,v),
            tr[nd]=min(tr[2*nd],tr[2*nd+1])
        );
    }

    void ad(ii l,ii r,ii v){
        l>r ? (void)0 : (void)ad(1,0,n,l,r,v);
    }
...
struct rs:sv{
    ...
    void go() override{
        ...
        fr(i,0,n){
            ...
            mx=max(mx,*reinterpret_cast<ii*>(&a[i]));
        }

        ii mv=max(n,mx);

        void* pc=new vi(mv+2,0);
        auto& cnt=cv<vi>(pc);
        ...
        fr(i,0,n){
            ...
            ii v=*reinterpret_cast<ii*>(&a[i]);
            (*reinterpret_cast<ii*>(&cnt[v]))++;

            (v<k && cnt[v]==1)
            ? (void)seg.ad(0,v,1)
            : (void)([&](){
                ii c=(v==0)?-1:(v-1)/2;
                c>=0 ? (void)seg.ad(0,min(n+1,c),1) : (void)0;
              })();
