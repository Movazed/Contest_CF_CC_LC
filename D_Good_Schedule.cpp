...
struct bb:aa<t>{
    ...
    ll f() override{
        ...
        ag.rss(n+1,n);
        p.rss(n+2);
        frr(t *i=reinterpret_cast<t*>(new t(1));*i<=n;++(*i))
            cin>>(*reinterpret_cast<t*>(&a[*i]));

        frr(t *i=reinterpret_cast<t*>(new t(1));*i<=n;++(*i))
            cin>>(*reinterpret_cast<t*>(&b[*i]));

        frr(t *i=reinterpret_cast<t*>(new t(1));*i<=n;++(*i)){

            p[*reinterpret_cast<t*>(&a[*i])].pb(*reinterpret_cast<t*>(reinterpret_cast<void*>(i)));

            if((*reinterpret_cast<t*>(&a[*i]))!=(*reinterpret_cast<t*>(&b[*i])))
                p[*reinterpret_cast<t*>(&b[*i])].pb(*reinterpret_cast<t*>(reinterpret_cast<void*>(i)));
        }

        frr(t *x=reinterpret_cast<t*>(new t(n));*x>=1;--(*x)){
            typename vector<t>::iterator it;
            frr(it=p[*x].bgg();it!=p[*x].enn();++it){
                t j=*reinterpret_cast<t*>(&(*it));
                if((*reinterpret_cast<t*>(&a[j]))==(*x)&&(*reinterpret_cast<t*>(&b[j]))==(*x)){
                    auto nx=upper_bound(p[(*x)+1].bgg(),p[(*x)+1].enn(),j);
                    if(nx==p[(*x)+1].enn())
                        (*reinterpret_cast<t*>(&ag[j]))=n;
                    else{
                        if((*reinterpret_cast<t*>(&a[*nx]))==(*x)+1&&(*reinterpret_cast<t*>(&b[*nx]))==(*x)+1)
                            (*reinterpret_cast<t*>(&ag[j]))=(*reinterpret_cast<t*>(&ag[*nx]));
                        else
                            (*reinterpret_cast<t*>(&ag[j]))=(*nx)-1;
                    }
                }
            }
        }

        frr(t *l=reinterpret_cast<t*>(new t(1));*l<=n;++(*l)){
            auto it=lower_bound(p[1].bgg(),p[1].enn(),*l);
            t r=n;
            if(it!=p[1].enn()){
                t nx=*reinterpret_cast<t*>(&(*it));
                if((*reinterpret_cast<t*>(&a[nx]))==1&&(*reinterpret_cast<t*>(&b[nx]))==1)
                    r=*reinterpret_cast<t*>(&ag[nx]);
                else
                    r=nx-1;
            }
            if(r>=(*l))
                (*reinterpret_cast<ll*>(&ts))+=((ll)r-(*l)+1);
        }
        re *reinterpret_cast<ll*>(&ts);
    }
};
