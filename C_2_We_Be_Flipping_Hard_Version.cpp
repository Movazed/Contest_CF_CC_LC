...
struct gg:ff{
    ...
    vector<vector<ll>> dp;
    vector<vector<ii>> yy,pp;

    gg(ii n){

        ll z=(ll)((~0ull)>>2);

        dp=vector<vector<ll>>(n+1,vector<ll>(3,-z));

        yy=vector<vector<ii>>(n+1,vector<ii>(3));

        pp=vector<vector<ii>>(n+1,vector<ii>(3));
    }
