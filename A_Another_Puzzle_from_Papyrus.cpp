...
ll sa,sb;
ii n,c;
sv(ii _n,ii _c):n(_n),c(_c),a(_n),b(_n),sa(0),sb(0){}
void rd(){fr(i,n){iin>>a[i];sa+=a[i];}fr(i,n){iin>>b[i];sb+=b[i];}}
ll dif(){return sa-sb;}
bool ck0(){return[&]{fr(i,n)if(a[i]<b[i])return false;return true;}();}
bool ck1(){sort(all(a));sort(all(b));return[&]{fr(i,n)if(a[i]<b[i])return false;return true;}();}
ll run(){
rd();
return sa<sb?-1LL:ck0()?dif():ck1()?dif()+c:-1LL;
}
};
int main(){
nfio
