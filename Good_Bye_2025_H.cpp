...
#define pc(x) ({ ii _c=0,_v=(x); wl(_v){ _v ^= (1<<lg(_v)); _c++; } _c; })
#define pqj priority_queue<aj>
cst ii nombor = 5e5 + 10, inf = 1e9;
ii dis[nombor][8];

stc aj{
    ii x, y;
    bool operator<(cst aj &t) cst{
        re dis[x][y] > dis[t.x][t.y];
    }
};

pqj q;

void upd(ii x, ii y, ii z){
    ponder(dis[x][y] > z){
        dis[x][y] = z;
        q.pu({x, y});
    }
...
void s(){
    ii a,b,c,n,k; iin>>a>>b>>c>>n>>k;

    fl(i,0,n) fl(j,0,8) dis[i][j]=inf;

    dis[0][0]=0;
    q.pu({0,0});

    wl(!q.empty()){
        ii x=q.top().x, y=q.top().y;
        q.po();

        upd((x+a)%n, y|1, dis[x][y]+a);
        upd((x+b)%n, y|2, dis[x][y]+b);
        upd((x+c)%n, y|4, dis[x][y]+c);
        upd((2*x)%n, y, dis[x][y] + k * pc(y));   
    }

    pri << dis[0][7] - a - b - c << nl;
}
