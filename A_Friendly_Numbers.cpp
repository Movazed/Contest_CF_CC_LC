...
#define fw(it,a) for(auto it=(a).begin();it!=(a).end();++it)
#define wh(c) while(c)

stc aa{virtual ll go(void*)=0;};
stc bb:aa{
    ll (*fn)(void*);
    ll go(void* p){re fn(p);}
};

template<class t> stc cc{ using tp=t; };
template<class t>
ll fd(void* p){
    t* q = static_cast<t*>(p);
    ll s=0,n=q->n;
...
ll gs(ll n){
    ...
    bb b;
    b.fn = &fd<dd>;
    re b.go(reinterpret_cast<void*>(&d));
}
