...
isprime[i*j]=false;}}}
#define FUNCTIONS_BLOCK \
unordered_map<ll,ll> mf,mh;\
\
ll gp2(ll x){ return 1LL << (63 - __builtin_clzll(x)); }\
\
ll H(ll m){\
    if(m==0) return 0;\
    auto it=mh.find(m);\
    if(it!=mh.end()) return it->second;\
    ll q=gp2(m);\
    return mh[m]=H(q-1)+H(m-q)+q;\
}\
\
ll F(ll n){\
    if(n<=1) return 0;\
    auto it=mf.find(n);\
    if(it!=mf.end()) return it->second;\
    ll p=gp2(n), m=n-p, c = m ? p : p+1;\
    return mf[n]=F(p-1)+H(m)+c;\
}
