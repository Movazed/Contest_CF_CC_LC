...
#define PROCESS_TEST_CASE \
    ...
    bool ok = (B[1] == 1) ? true : false; \
    i = 1; \
    while (i <= N && ok) { \
        ok = (B[i] >= 1 && B[i] <= i) ? true : false; \
        if (i > 1) { \
            ok = (B[i] <= B[i - 1] + 1) ? ok : false; \
        } \
        i++; \
    } \
    if (!ok) { \
    ...
    while (i <= N) { \
        int d = B[i]; \
        while ((int)st.size() > d) st.pop_back(); \
        int p = st.back(); \
    ...
    while (!stk.empty()) { \
        auto &cur = stk.back(); \
        int u = cur.first, idx = cur.second; \
    ...
    for (int u : order) { \
        ...
        vector<int> F(K + 1, 1), Fn(K + 1, 0), S(K + 1, 0), P(K + 1, 0); \
        for (int v : children[u]) { \
            int t = 1; \
            while (t <= K) { \
                S[t] = mulmod(dp[v][t], F[t - 1]); \
                t++; \
            } \
            long long run = 0; \
            P[0] = 0; \
...
#define addmod(a, b) ({ int s = (a) + (b); (s >= MOD) ? (s - MOD) : s; })
