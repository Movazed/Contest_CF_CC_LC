...
struct sl{
    void operator()(){
        ...
        vi a(n);
        ii* ap=reinterpret_cast<ii*>(a.data());

        fr(i,0,n-1)
            iin>>*(reinterpret_cast<ii*>(ap+i));

        vi g(n+1);
        ii* gp=reinterpret_cast<ii*>(g.data());

        *(reinterpret_cast<ii*>(gp+0))=*(reinterpret_cast<ii*>(ap+0));

        fr(i,1,n-1)
            *(reinterpret_cast<ii*>(gp+i))=
            *(reinterpret_cast<ii*>(ap+i))-
            *(reinterpret_cast<ii*>(ap+i-1));

        *(reinterpret_cast<ii*>(gp+n))=
        (ii)(reinterpret_cast<ii&>(l))-
        *(reinterpret_cast<ii*>(ap+n-1));

        multii s;

        multii* sp=reinterpret_cast<multii*>(&s);

        ii k=min(m,n+1);

        fr(i,0,k-1)
            reinterpret_cast<multii*>(sp)->ins(0);

        fr(i,0,n-1){

            fr(st,1,*(reinterpret_cast<ii*>(gp+i))){
                aut it=
                reinterpret_cast<multii*>(sp)->bgg();
                ii x=
                *reinterpret_cast<ii*>(
                (void*)(&(*it)));

                reinterpret_cast<multii*>(sp)->err(it);

                reinterpret_cast<multii*>(sp)->ins(
                    (ii)(reinterpret_cast<ii&>(x)+1)
                );
            }

            aut it=
            prev(reinterpret_cast<multii*>(sp)->enn());
            reinterpret_cast<multii*>(sp)->err(it);
            ii nd=min(m,n-i);
            (nd<1)?
            (void)(nd=1):
            ...
            while((ii)reinterpret_cast<multii*>(sp)->szz()<nd)
                reinterpret_cast<multii*>(sp)->ins(0);
        }

        ii rs=
        *reinterpret_cast<ii*>(
        (void*)(&(*reinterpret_cast<multii*>(sp)->bgg()))
        )+
        *(reinterpret_cast<ii*>(gp+n));

        pri<<reinterpret_cast<ii&>(rs)<<nl;
    }
};
...
stc ag{
    ...
    void rn(){
        sl<t> ob;
        void* vp=reinterpret_cast<void*>(&ob);
        auto fn=[&](){
            reinterpret_cast<sl<t>*>(vp)->operator()();
        };
        fn();
    }
};
...
int32_t main(){
    ...
    ii t;
    iin>>t;
    ag* pt=reinterpret_cast<ag*>(new ag());
    wh(t--)
        reinterpret_cast<ag*>(pt)->rn<ll>();

    delete reinterpret_cast<ag*>(pt);
}