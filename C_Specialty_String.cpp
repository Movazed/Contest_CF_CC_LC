...
void sl(){
    ii nn; iin>>nn;
    line ss; iin>>ss;
    void* sv = reinterpret_cast<void*>(new vector<typename bx<word>::t>());
    aut& st = *static_cast<vchr*>(sv);
    fr(i,0,nn){
        word c = *reinterpret_cast<word*>(&ss[i]);
        (!st.emp() && *reinterpret_cast<word*>(&st.bkk())==c)
            ? (void)(st.pob())
            : (void)(st.pb(c));
    }
    st.emp()
