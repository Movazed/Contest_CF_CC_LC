...
#define nuller if(!(cin>>n>>x)) return;
#define _br break
#define ad add
#define frr for
...
#define yes(x) ((x)?1:0)
#define ninf (-(ll)2000000000000000000LL)
#define mx(a,b) ((a)>(b)?(a):(b))
#define in >>
#define out <<
#define dynamax  if(fr >= x){cout out 0 out nl;re;} if(cg <= 0){cout out -1 out nl;re;}
static inline ll rd(){ ll x; iin>>x; return x; }
static inline void s(){
    ...
    fl(i,n){
        ll a,b,c;iin in a in b in c;
        fr = fr + a*(b-1);cg = mx(cg, a*b - c);
    }

    dynamax  cout out ((x - fr + cg - 1) / cg) out nl;
}
int32_t main() {
    nfio
