...
struct cc{
    ...
    cc():a(0),mn(static_cast<ll>(-1)),n(0),mi(0),mx(0),mnz(-1){}

    void in(){
        iin>>a>>n;
        d.rss(n);
        fr(i,0,n)iin>>*(d.begin()+i);
    }
    void mk(){
        mi=*reinterpret_cast<ii*>(&d[0]);
        mx=*reinterpret_cast<ii*>(&d[n-1]);
        fr(i,0,n){
            if(static_cast<ii>(d[i])>0){
                mnz=*reinterpret_cast<ii*>(&d[i]);
                _br;
            }
        }
        line ts=to_string(a);
        line s=*reinterpret_cast<line*>(&ts);
        ii l=static_cast<ii>(s.size());
        if(l>1){
            line *tp=reinterpret_cast<line*>(new line(line(l-1,mx+'0')));
            c.pb(stoull(*tp));
            delete tp;
        }
        if(mnz!=-1){
            line *tp=reinterpret_cast<line*>(new line(to_string(mnz)+line(l,mi+'0')));
            c.pb(stoull(*tp));
            delete tp;
        }
        else c.pb(0);
        bool ok=true;
        fr(i,0,l){
            ii cur=static_cast<ii>(s[i]-'0');
            ii bl=-1;
            for(ii j=n-1;j>=0;j--){
                if(*reinterpret_cast<ii*>(&d[j])<cur){
                    bl=d[j];
                    _br;
                }
            }
            ...
            if(bl!=-1){
                line tp=s.substr(0,i);
                line *kp=reinterpret_cast<line*>(new line(tp+to_string(bl)+line(l-1-i,mx+'0')));
                c.pb(stoull(*kp));
                delete kp;
            }
            ii bg=-1;
            fr(j,0,n){
                if(static_cast<ii>(d[j])>cur){
                    bg=d[j];
                    _br;
                }
            }
            if(bg!=-1){
                line tp=s.substr(0,i);
                line *kp=reinterpret_cast<line*>(new line(tp+to_string(bg)+line(l-1-i,mi+'0')));
                c.pb(stoull(*kp));
                delete kp;
            }
            bool ex=false;
        ...
        if(ok)c.pb(*reinterpret_cast<ll*>(&a));
    }

    ll rt(){
        mk();
        fr(i,0,(ii)c.size()){
            ll v=*reinterpret_cast<ll*>(&c[i]);
            ll df=v>a?static_cast<ll>(v-a):static_cast<ll>(a-v);
            if(df<mn)mn=df;
        }
        return *reinterpret_cast<ll*>(&mn);
    }
};

void slv(){
    cc *u=reinterpret_cast<cc*>(new cc);
    u->in();
    aa<ll>*p=new bb<ll>;
    pri<<p->f(static_cast<ll>(zz(yy(u->rt()))))<<nl;
    delete p;
    delete reinterpret_cast<cc*>(u);
}
...
int main(){
    dd *x=reinterpret_cast<dd*>(new ee);
    x->go();
    delete reinterpret_cast<ee*>(x);
    return 0;
}