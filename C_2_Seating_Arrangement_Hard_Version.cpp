...
struct bb:aa<t>{
    t q(t x){
        return reinterpret_cast<t(*)(t)>(+[](t y)->t{return y;})(x);
    }
};
...
struct iiq:hh{
    ...
    void z(){
        ...
        iin>>*reinterpret_cast<ll*>(nw)
           >>*reinterpret_cast<ll*>(xw)
           >>*reinterpret_cast<ll*>(sw);

        line u;
        iin>>u;

        ll et=ee(*reinterpret_cast<ll*>(xw),*reinterpret_cast<ll*>(nw));
        ll as=0,ae=0,ans=0;

        frr(aut it=reinterpret_cast<line*>(&(u))->bgg();
            it!=reinterpret_cast<line*>(&(u))->enn();
            ++it){

            word&p=*reinterpret_cast<word*>(&(*it));

            (p=='I')?
            (
                et>0?
                (--et,as+=(*reinterpret_cast<ll*>(sw)-1),++ans)
                :
                0
            )
            :
            (
                ...
                (
                    ...
                    (
                        ...
                        (
                            et>0&&ae>0?
                            (--et,as+=(*reinterpret_cast<ll*>(sw)-1),--ae,++ans)
                            :
                            0
                        )
                    )
...
int main(){
    ...
    unique_ptr<hh>u=
    static_cast<unique_ptr<hh>>(
        make_unique<iiq>()
    );

    ii*tw=new ii;
    cin>>*reinterpret_cast<ii*>(tw);

    while((*reinterpret_cast<ii*>(tw))--)

        ([&](){

            reinterpret_cast<void(*)(hh*)>(
                +[](hh*x){
                    x->z();
                }
            )(u.get());
