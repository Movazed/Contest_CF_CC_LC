...
template<class t>
stc tb{
    template<class u> stt u mn(u a,u b){return a<b?a:b;}
    template<class u> stt u mx(u a,u b){return a>b?a:b;}
    template<class u> stt u ap(u a,u b){return a+b;}
};
stc di{
    ll m1=-1e18,m2=-1e18;
    ii c1=0;
...
stc ba{virtual void rn(void*)=0; virtual ~ba(){}};
stc sv:ba{
    void sl(){
        ii n,m; iin>>n>>m;
        vvl a(n,vll(m));
        fr(i,n) fr(j,m){
            ll* p=reinterpret_cast<ll*>(&a[i][j]);
            iin>>*p;
        }
        vvl dp(n,vll(m,-1e18));
        vvl rd(n,vll(m,-1e18));
        fr(i,n) fr(j,m){
            ll pv=(i==0 && j==0)?0:-1e18;
            pv=i>0?tb<ll>::mx(pv,dp[i-1][j]):pv;
            pv=j>0?tb<ll>::mx(pv,dp[i][j-1]):pv;
            dp[i][j]=(pv==-1e18?0:pv)+a[i][j];
        }
        frd(i,n-1,0) frd(j,m-1,0){
            ll nv=(i==n-1 && j==m-1)?0:-1e18;
            nv=i+1<n?tb<ll>::mx(nv,rd[i+1][j]):nv;
            nv=j+1<m?tb<ll>::mx(nv,rd[i][j+1]):nv;
            rd[i][j]=a[i][j]+(nv==-1e18?0:nv);
        }
        ll ob=dp[n-1][m-1], an=ob;
        ii md=n+m-2;
        vector<di> dg(md+1);

        fr(i,n) fr(j,m){
            ll pv=dp[i][j]+rd[i][j]-a[i][j];
            ii d = i + j;

            ll* pm1 = reinterpret_cast<ll*>(&dg[d].m1);
            ll* pm2 = reinterpret_cast<ll*>(&dg[d].m2);
            ii* pc1 = reinterpret_cast<ii*>(&dg[d].c1);
            ll* pv1 = reinterpret_cast<ll*>(&dg[d].v1);
            (pv > *pm1) ?
            ([&](){
                *pm2 = *pm1;
                *pm1 = pv;
                *pc1 = 1;
                *pv1 = *reinterpret_cast<ll*>(&a[i][j]);
            }(), void()) 
            : (pv == *pm1) ?
            ([&](){ (*pc1)++; }(), void()) 
            : (pv > *pm2) ?
        ...
        fr(d,md+1){
            di* pd=reinterpret_cast<di*>(&dg[d]);
            (pd->c1==1 && pd->m1==ob) ?
            ([&](){
                an=tb<ll>::mn(an,tb<ll>::mx(pd->m1-2*pd->v1,pd->m2));
            }(), void()) :
            void();
        }
        pri<<an<<nl;
    }
    static void th(void* p){ static_cast<sv*>(p)->sl(); }
    void rn(void* p){ auto f=reinterpret_cast<void(*)(void*)>(p); f(this); }
};
