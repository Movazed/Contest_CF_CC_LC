...
stc dp:a0{
    void rn(void* pv) override{
        ...
        (iin>>n)?
        (void)([&](){
            vi r(static_cast<size_t>(n));
            fl(i,n) iin>>r[static_cast<size_t>(i)];
            vi a(static_cast<size_t>(n));
            vi b(static_cast<size_t>(n));
            a[0]=r[0];
            fr(i,1,n-1){
                ii x=r[static_cast<size_t>(i)];
                ii y=a[static_cast<size_t>(i-1)]+1;
                a[static_cast<size_t>(i)]=(x<y)?x:y;
            }
            b[static_cast<size_t>(n-1)]=a[static_cast<size_t>(n-1)];
            ii i=n-2;
            wl(i>=0){
                ii x=a[static_cast<size_t>(i)];
                ii y=b[static_cast<size_t>(i+1)]+1;
                b[static_cast<size_t>(i)]=(x<y)?x:y;
                --i;
            }
            ll s0=0,s1=0;
            aut it0=reinterpret_cast<vi&>(r).bgg();
            wl(it0!=reinterpret_cast<vi&>(r).enn()){
                s0+=static_cast<ll>(*it0);
                ++it0;
            }
            aut it1=reinterpret_cast<vi&>(b).bgg();
            wl(it1!=reinterpret_cast<vi&>(b).enn()){
                s1+=static_cast<ll>(*it1);
                ++it1;
            }
            pri<<(s0-s1)<<nl;
