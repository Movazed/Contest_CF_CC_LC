...
#define INIT_VARS(N, M, res) \
    ll N, M; cin >> N >> M; \
    string res; res.reserve((size_t)(2 * (N + M) - 1))

#define BUILD_STRING(N, M, res) ((N == 0) ? ([&]() { \
    ll i = 0; \
    while (i < (M) - 2) { res.pb('>'); res.pb('<'); i++; } \
    res.pb('>'); res.pb('='); res.pb('>'); \
}()) \
: ((M) == 0 ? ([&]() { \
    ll i = 0; \
    while (i < (N) - 2) { res.pb('<'); res.pb('<'); i++; } \
    res.pb('<'); res.pb('='); res.pb('<'); \
}()) \
: ([&]() { \
    ll i = 0; \
    while (i < 2 * (N) - 1) { res.pb('<'); i++; } \
    ll j = 0; \
    while (j < (M)) { res.pb('<'); res.pb('>'); j++; } \
}())))

void solve() {
    INIT_VARS(N, M, res);

    BUILD_STRING(N, M, res);
