...
#define READ_ARRAY_AND_COMPUTE(n, arr, sum, maxVal) \
    {                                              \
        ll i = 0;                                  \
        while (i < n) {                            \
            cin >> arr[i];                         \
...
void solve() {
    ...
    READ_ARRAY_AND_COMPUTE(n, a, sumA, maxA);
    READ_ARRAY_AND_COMPUTE(n, b, sumB, maxB);
    ...
    if (diff == 0) {
        cout << (a == b ? (maxA + 1 <= 1000000000LL ? maxA + 1 : -1) : -1) << "\n";
        return;
    }
    ...
    while (d * d <= diff) {
        if (diff % d == 0) {
            ...
            (d * d != diff) ? divisors.push_back(diff / d) : void();
        }
        d++;
    }

    ll idx = 0;
    while (idx < divisors.size()) {
        ll k = divisors[idx];
        if (k <= maxB) {
            idx++;
            continue;
