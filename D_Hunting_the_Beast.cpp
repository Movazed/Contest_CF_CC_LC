...
#define wlh while
#define sof sizeof
#define rp(x,y,z) for(ii x=y;x<=z;x++)
#define rpd(x,y,z) for(ii x=y;x>=z;x--)
#define md 998244353
#define mx 1000005
ll fc[mx],ifc[mx];
ll pw(ll bs,ll ep){
    ...
    bs<0?(bs+=md,0):0;
    while(ep>0){
        ep%2==1?(rs=rs*bs%md,0):0;
        bs=bs*bs%md;
        ep/=2;
    }
    return rs;
}
function<ll(ll)>mi=[](ll n)->ll{return pw(n,md-2);};
void pc(){
    fc[0]=ifc[0]=1;
rp(i,1,mx-1)*reinterpret_cast<ll*>(&fc[i])=fc[i-1]*i%md;
ii tmp=mx-1;
ifc[tmp]=mi(*reinterpret_cast<ll*>(&fc[*reinterpret_cast<ii*>(&tmp)]));
rpd(i,mx-2,1)ifc[i]=*reinterpret_cast<ll*>(&ifc[i+1])*(i+1)%md;
    rpd(i,mx-2,1)ifc[i]=ifc[i+1]*(i+1)%md;
}
function<ll(ii,ii)>cr=[](ii n,ii r)->ll{
    return(r<0||r>n)?0:fc[n]*ifc[r]%md*ifc[n-r]%md;
};
void solve(){
    ii n,m;iin>>n>>m;
    ll an=0;
    rp(k,m,n){
        if(k==0||*(ii*)&k==0)continue;
        ll ck=k==1?0:pw(k-1,k)*ifc[k]%md;
        if(ck==0&&k!=n)continue;
        ll t1=n>=k?pw(md+1-k,n-k)*ifc[n-k]%md:0;
        ll t2=n>=k+1?pw(md+1-k,n-k-1)*ifc[n-k-1]%md:0;
        ll pk=(t1-t2+md)%md*fc[n]%md;
        ll ct=ck*pk%md*cr(k,m)%md;
        an=(an+ct)%md;
    }
    (m==n&&n==1)?(an=1,0):0;
    pri<<an<<nl;
