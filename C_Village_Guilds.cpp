...
#define rb(i,a,b) for(ii i=a;i>=b;i--)
struct sv{
ii n;
vi p,m1,m2,md;
sv():n(0){
iin>>n;
p.rss(n+1);m1.rss(n+1);m2.rss(n+1);md.rss(n+1);
fill(all(m1),-1);fill(all(m2),-1);fill(all(md),0);
fr(i,2,n)iin>>p[i];
}
ll run(){
rb(i,n,2){
ii pr=p[i],vl=md[i];
vl>m1[pr]?(m2[pr]=m1[pr],m1[pr]=vl):vl>m2[pr]?m2[pr]=vl:0;
md[pr]=m1[pr]+1;
}
ll an=n;
fr(i,1,n)m2[i]!=-1?an+=m2[i]+1:0;
return an;
}
};
int main(){
