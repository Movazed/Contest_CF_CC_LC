...
#define lr(i,a,b) for(ii i=(a);i>=(b);--i)
#define it(i,a,b) for(auto i=(a);i!=(b);++i)
...
void sl(rd rr, wr ww){
    ...
    lp(i,0,nn){
        ii llx;
        rr(llx) ? (void)0 : (void)0;
        vi bl(llx);
        lp(j,0,llx) rr(bl[j]) ? (void)0 : (void)0;
        vi td;
        lr(j,llx-1,0){
            se[bl[j]] ? (void)0 :
            (void)(se[bl[j]]=true,ss[i].push_back(bl[j]),td.push_back(bl[j]));
        }
        it(p,td.begin(),td.end()) se[*p]=false;
    }
    vbo us(nn,false);
    vi qq;
    vi gd;
    lp(st,0,nn){
        ...
        lp(i,0,nn){
            ...
            (void)([&](){
                vi cs;
                it(p,ss[i].bgg(),ss[i].end())
                    se[*p] ? (void)0 : (void)cs.pb(*p);

                (bi==-1 || cs<bs) ?
                    (void)(bi=i,bs=std::move(cs)) :
                    (void)0;
            })();
        }
        us[bi]=true;
    ...
    lp(i,0,(ii)qq.size()){
        ww(qq[i]);
        (i+1==(ii)qq.size()) ? ww(-1) : ww(-2);
    }
    it(p,gd.bgg(),gd.end()) se[*p]=false;
}

stc io {
    bool operator()(ii &x){
        return (iin>>x)?true:false;
    }
    void operator()(ii x){
        x==-1 ? (cout<<"\n") : (x==-2 ? (cout<<" ") : (cout<<x));
    }
...
void oox(){
    ...
    iin>>tt;
    io fn;
    ab<ii>* bp = reinterpret_cast<ab<ii>*>(new vb());
    ii zz = static_cast<vb*>(bp)->go(0);
    (void)zz;
    fx< function<bool(ii&)> > rd{
        function<bool(ii&)>([&](ii& x){
            return (fn.*static_cast<bool (io::*)(ii&)>(&io::operator()))(x);
        })
    };
    fx< function<void(ii)> > wr{
        function<void(ii)>([&](ii x){
            (fn.*static_cast<void (io::*)(ii)>(&io::operator()))(x);
        })
    };
    lp(i,0,tt) sl(rd,wr);
    delete reinterpret_cast<vb*>(bp);
}
