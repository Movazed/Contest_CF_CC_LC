...
#define wl(x) while(x)
#define revz__         A.revz(2000000);B.revz(4000000);
const ii md=998244353;

ll pw(ll b,ll e){
    ll r=1; b%=md;
    wl(e)
        (e&1)?(void)(r=r*b%md):(void)0,
        b=b*b%md,
        e>>=1;
    return r;
}

ll iv(ll x){ return pw(x,md-2); }

struct fx{
    ii n,d;
    bool operator<(const fx& o) const{
        auto p=reinterpret_cast<const ii*>(this);
        auto q=reinterpret_cast<const ii*>(&o);
        return 1LL*(*(p+0))*(*(q+1)) < 1LL*(*(q+0))*(*(p+1));
    }
};

vector<fx> A,B;

struct sv{ virtual void go()=0; };
...
struct rs:sv{
    ...
    void go() override{
        ...
        fr(i,0,n) iin>>(*(reinterpret_cast<ii*>(&(a[i]))));
        fr(i,0,n) iin>>(*(reinterpret_cast<ii*>(&(b[i]))));
        ...
        : (void)([&](){
            ...
            fr(i,0,n)
                fr(j,i+1,n)
                    (a[i]>a[j])?(void)(++ia):(void)0;

            ll tm = tot%md;
            ll eq = (1LL*n*(ia%md))%md;
...
struct im:ex{
    ...
    void rn() override{
        ...
        void* p=new rs;
        sv* b=reinterpret_cast<sv*>(p);

        wl(t--)
            b->go();

        delete reinterpret_cast<rs*>(p);
    }
};
