...
#define fr(i,n) for(ii i=0;i<(ii)(n);++i)

template<class tt>
stc ab{ static tt go(const tt& x){ return x; } };

stc vb{ virtual ll ck(void*,void*)=0; };

stc dv:vb{
    ll (*fn)(void*,void*);
    dv(ll (*f)(void*,void*)) : fn(f) {}
    ll ck(void* a,void* b){ return fn(a,b); }
};

static ll gf(void* ap,void* bp){
    vll* av = reinterpret_cast<vll*>(ap);
    pll_* pr = reinterpret_cast<pll_*>(bp);
    ll x = pr->frs;
    ll y = pr->scs;
    ll tg = 0;
    aut it = av->bgg();
...
void slv(){
    ...
    iin>>n>>x>>y;
    vll a((size_t)n);
    fr(i,n) iin>>a[(size_t)i];
    pll_ pr = {ab<ll>::go(x),ab<ll>::go(y)};
    vb* ob = new dv(&gf);
    ll an = ob->ck(reinterpret_cast<void*>(&a),
