...
class ss : public sb {
    ...
    void rd() {
        iin >> n;
        a = static_cast<ll*>(malloc(sizeof(ll)*n));
        b = static_cast<ll*>(malloc(sizeof(ll)*n));
        f(i,n) {
            ll t;
            iin >> t;
            *(a+i) = static_cast<ll>(t);
        }
        f(i,n) {
            ll t;
            iin >> t;
            *(b+i) = static_cast<ll>(t);
        }
    }
public:
    void sv() override {
        ...
        f(k,n) {
            ...
            if(s>n) break;
            ll c = *(a+s-1);
            mx = max(mx, static_cast<ll>(c*static_cast<ll>(k+1)));
        }
        pri << reinterpret_cast<ll&>(mx) << nl;
        free(a);
        free(b);
    }
};
...
int32_t main() {
    ...
    iin >> t;
    f(_,t) {
        sb* x = reinterpret_cast<sb*>(malloc(sizeof(ss)));
        x = new(x) ss();
        x->sv();
        x->~sb();
        free(x);
