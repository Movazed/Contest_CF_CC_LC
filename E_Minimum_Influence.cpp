...
struct bb:aa<t>{
    t n,m;
    vll p,c,mc,mp,tp,tc,d;
    ll gmp,gmc,gms;
    bb():n(0),m(0),gmp(I),gmc(I),gms(I){}
    ...
    void f() override{
        ...
        mc.rss(M,I);
        mp.rss(M,I);
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<n;++(*i))
            iin>>(*reinterpret_cast<ll*>(&p[*i]));
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<n;++(*i))
            iin>>(*reinterpret_cast<ll*>(&c[*i]));
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<n;++(*i)){
            mc[*reinterpret_cast<ll*>(&p[*i])]=min(mc[*reinterpret_cast<ll*>(&p[*i])],c[*i]);
            mp[*reinterpret_cast<ll*>(&c[*i])]=min(mp[*reinterpret_cast<ll*>(&c[*i])],p[*i]);
            gmp=min(gmp,p[*i]);
            gmc=min(gmc,c[*i]);
            gms=min(gms,p[*i]+c[*i]);
        }
        frr(t *i=reinterpret_cast<t*>(new t(1));*i<M;++(*i)){
        ...
        tc.rss(m);
        d.rss(m);
        frr(t *i=reinterpret_cast<t*>(new t(0));*i<m;++(*i))

            iin>>(*reinterpret_cast<ll*>(&tp[*i]));

        frr (t *i=reinterpret_cast<t*>(new t(0));*i<m;++(*i))

            iin>>(*reinterpret_cast<ll*>(&tc[*i]));

        frr(t *i=reinterpret_cast<t*>(new t(0));*i<m;++(*i))

            iin>>(*reinterpret_cast<ll*>(&d[*i]));

        frr(t *j=reinterpret_cast<t*>(new t(0));*j<m;++(*j)){
            ll P=tp[*j]+d[*j];
            ll C=tc[*j]+d[*j];
            ll b1=I,b2=I;
            if(tp[*j]>0){
                t id=min((ll)M-1,tp[*j]-1);
                ll x=*reinterpret_cast<ll*>(&mc[id]);
                if(x<tc[*j])
                    b1=0;
                else if(x!=I)
                    b1=min(x,C);
            ...
            if(tc[*j]>0){
                t id=min((ll)M-1,tc[*j]-1);
                ll x=*reinterpret_cast<ll*>(&mp[id]);
                if(x<tp[*j])
                    b2=0;
                else if(x!=I)
                    b2=min(x,P);
