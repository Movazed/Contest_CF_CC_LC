...
#define fr0(i,a,b) for(ii i=(a);i<(b);++i)
#define frd(i,a,b) for(ii i=(a);i>=(b);--i)

stc fnc{ ll a,b,c; ii id; };
template<class tt>
stc ab{
...
template<class tt>
stc ac:ab<tt>{
    void go(void* vp) override{
        aut lm = [&](void* qp){
            aut* tp = reinterpret_cast<tuple<ii*,vector<fnc>*,vi*>*>(qp);
            ii* n = get<0>(*tp);
            aut* f = get<1>(*tp);
            aut* an = get<2>(*tp);
            aut ls = [&](cst fnc& u,cst fnc& v)->bool{
                re (u.a==v.a)?
                       ((u.b==v.b)&&(u.c<v.c)):
                       (u.a<v.a)?((u.b-v.b)*(u.b-v.b)< 4ll*(v.a-u.a)*(v.c-u.c)):0;
            };
            sort(f->bgg(),f->enn(),[&](cst fnc& x,cst fnc& y){
                     re (x.a!=y.a)?x.a<y.a: (x.c!=y.c)?x.c<y.c:x.b<y.b;});
            vi di(*n,1),do2(*n,1);
            fr0(i,0,*n)
                fr0(j,0,i)
                    ls((*f)[j],(*f)[i])?
                        di[i]=max(di[i],di[j]+1):0;
            frd(i,(*n)-1,0)
                fr0(j,i+1,*n)
                    ls((*f)[i],(*f)[j])?
                        do2[i]=max(do2[i],do2[j]+1):0;
            fr0(i,0,*n)
                (*an)[(*f)[i].id]=di[i]+do2[i]-1;
        };
        lm(vp);
    }
};
...
stc sv:so{
    void run() override{
        ...
        vi an(n);
        aut tp = make_tuple(&n,&f,&an);
        ab<ii>* ob = static_cast<ab<ii>*>(new ac<ii>());
        function<void(void*,ab<ii>*)> fn = [](void* p,ab<ii>* o){ o->go(p); };
        fn(reinterpret_cast<void*>(&tp),ob);
        fr0(i,0,n)
            pri<<an[i]<<(i+1==n?'\n':' ');
        delete ob;
    }
...
int32_t main(){
    ...
    (
        [&](){
            wl(t--){
                so* oo = static_cast<so*>(new sv());
                function<void(void*)> fx = [](void* p){
                        reinterpret_cast<so*>(p)->run();
                    };
                fx(reinterpret_cast<void*>(oo));
                delete oo;
            }
        }(),0
    ):0;
