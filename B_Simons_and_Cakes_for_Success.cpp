...
stc im:ab<ll>{
    ll go(ll v) override { return v; }
};

template<class f>
stc fx{
    f fn;
    ll operator()(ll x){ return fn(x); }
};
...
void sl(){
    ...
    ll* pp=reinterpret_cast<ll*>(vp);
    stt im dv;
    ab<ll>* bd=static_cast<ab<ll>*>(&dv);
    aut gd=[&](ll x)->ll{ re bd->go(x); };
    aut wk=[&](ll i)->ii{
        wh((*pp)%i==0) *pp/=i;
        re 0;
    };
...
int32_t main(){
   nfio
    ii tc;
    iin>>tc;
    vector<function<void()>> oops;
    oops.pb([](){
        sl();
    });
    fr(i,0,tc){
        aut* fp=reinterpret_cast<function<void()>*>(&oops[0]);
        (*fp)();
    }
    re 0;
}