...
struct st{
    ii n;
    vi mn, mdi;
    st(ii s):n(s){mn.rez(4*n+5);mdi.rez(4*n+5);
    fill(all(mn), infy);fill(all(mn), infy);
    }
    void up(ii v,ii l,ii r,ii i,ii x){
    if(l==r){ mn[v]=x, mdi[v]=x-l; re; }
    ii m=(l+r)>>1;
    (i<=m)?up(v<<1,l,m,i,x):up(v<<1|1,m+1,r,i,x);
    mn[v]=min(mn[v<<1],mn[v<<1|1]);
    mdi[v]=min(mdi[v<<1],mdi[v<<1|1]);
    }

    ii qmn(ii v,ii l,ii r,ii L,ii R){
    if(R<l||r<L) re infy;
    if(L<=l&&r<=R) re mn[v];
    ii m=(l+r)>>1;
    re min(qmn(v<<1,l,m,L,R),qmn(v<<1|1,m+1,r,L,R));
    }
    ii ff(ii v,ii l,ii r,ii L,ii R,ii x){
    if(R<l||r<L||mdi[v]>x) re -1;
    if(l==r) re l;
    ii m=(l+r)>>1,res;
    re ((res=ff(v<<1,l,m,L,R,x))!=-1)?res:ff(v<<1|1,m+1,r,L,R,x);}};
    void sl(){
    ii n,q; iin>>n>>q;
    st t(n);
    fl(i,1,n){ii x; iin>>x;t.up(1,1,n,i,x);}
    wl(q--){ii tp; iin>>tp;
    if(tp==1){ii i,x; iin>>i>>x;t.up(1,1,n,i,x);
    }else{ii l,r; iin>>l>>r;ii p=t.ff(1,1,n,l,r,-l);if(p==-1) pri<<"0\n";
    else{ii d=p-l;ii mv=t.qmn(1,1,n,l,p);pri<<(mv==d?"1\n":"0\n");}}}}
