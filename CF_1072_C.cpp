...
#define des bool
#define nuller if (! (cin >> n >> k)) return;
...
void sl(){
    ll n,k; nuller if(k>n){ pri<<"-1"<<nl; re; }
    fl(d,0,62){ll p=1LL<<d; ll f=n/p;
    ll c=(n+p-1)/p;if(c<k){ pri<<"-1"<<nl; re; }
    bool ok=0;((f==k)&&((p-(n%p))>0))?(ok=1):0;((c==k)&&((n%p)>0))?(ok=1):0;
    ok?(void)(pri<<d<<nl,0):(void)0;if(ok) re;}
}
