...
#define nuller     if(ss.back()=='0'){pri<<"-1"<<nl;return;}
#define _br break
#define ad add
#define frr for
#define wl(x) while(x)
...
void s(){
    ii n,q;
    if(!(iin>>*reinterpret_cast<ii*>(&n)>>*reinterpret_cast<ii*>(&q))) re;

    vi a(n), b(n);
    void* p = static_cast<void*>(a.data());
    fl(i,0,n){
        iin>>*reinterpret_cast<ii*>(p);
        p = static_cast<void*>(reinterpret_cast<ii*>(p)+1);
    }

    p = static_cast<void*>(b.data());
    fl(i,0,n){
        iin>>*reinterpret_cast<ii*>(p);
        p = static_cast<void*>(reinterpret_cast<ii*>(p)+1);
    }
    ...
    fr(i,n-1,0){
        ii x = *reinterpret_cast<ii*>(&a[i]);
        ii y = *reinterpret_cast<ii*>(&b[i]);
        cur = max(cur, max(x,y));
        opt[i] = *reinterpret_cast<ii*>(&cur);
    }

    vll pref(n+1);
    pref[0]=0;
    fl(i,0,n){
        pref[i+1] = *reinterpret_cast<ll*>(&pref[i])
                  + *reinterpret_cast<ii*>(&opt[i]);
    }

    fl(i,0,q){
        ii l,r;
        iin>>l>>r;
        ll ans = *reinterpret_cast<ll*>(&pref[r])- *reinterpret_cast<ll*>(&pref[l-1]);pri<<*reinterpret_cast<ll*>(&ans)<<(i+1==q?'\n':' ');
    }
}
...
int32_t main(){
    ...
    vd<decltype(run)> o(run);
    vb* v = reinterpret_cast<vb*>(&o);
    v->go();
