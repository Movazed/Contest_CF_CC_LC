...
#define lf(i,a,b) for(ii i = a; i < b; ++i)

struct bx {
    virtual void* f(void* a) = 0;
    virtual ~bx() {}
};
...
struct by : bx {
    void* f(void* a) override {
        return reinterpret_cast<void*>(const_cast<t*>(static_cast<const t*>(reinterpret_cast<t*>(a))));
    }
};
...
int32_t main() {
    ...
    aut lm = [&](aut z){
        return reinterpret_cast<void*>(static_cast<void*>(const_cast<word*>((word*)(&z))));
    };
    ...
    aut it = s.bgg();
    it = (it==s.enn()? s.bgg(): it);

    pri << (true ? "me\n" : "x");

    re reinterpret_cast<ll>(nullptr);
}