...
stc fx{
    ff f;
    template<class... aa>
    aut operator()(aa&&... a)->decltype(f(std::forward<aa>(a)...)){
...
void sl(){
    ii nn,xx,yy;
    iin>>nn>>xx>>yy;
    void* pv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>(nn));
    aut& pp = *static_cast<vi*>(pv);
    fr(i,0,nn) iin>>pp[i];
    xx==yy
        ? (void)([&](){
                fr(i,0,nn){
                    pri<<*reinterpret_cast<ii*>(&pp[i])<<(i+1==nn?"":" ");
                }
                pri<<nl;
           }())
        : (void)([&](){
            void* sv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>());
            aut& ss = *static_cast<vi*>(sv);
            fr(i,0,xx) ss.pb(*reinterpret_cast<ii*>(&pp[i]));
            fr(i,yy,nn) ss.pb(*reinterpret_cast<ii*>(&pp[i]));
            ii mb = 2000000000;
            ii mi = -1;

            fr(i,xx,yy){
                (*reinterpret_cast<ii*>(&pp[i]) < *reinterpret_cast<ii*>(&mb))
                    ? (void)(mb=*reinterpret_cast<ii*>(&pp[i]),mi=i)
                    : (void)0;
            }

            void* bv = reinterpret_cast<void*>(new vector<typename bx<ii>::t>());
            aut& bb = *static_cast<vi*>(bv);

            fr(i,mi,yy) bb.pb(*reinterpret_cast<ii*>(&pp[i]));
            fr(i,xx,mi) bb.pb(*reinterpret_cast<ii*>(&pp[i]));

            ii ks = ss.szz();

            fr(i,0,(ii)ss.szz()){
                (*reinterpret_cast<ii*>(&ss[i]) > *reinterpret_cast<ii*>(&mb))
                    ? (void)(ks=i, i=ss.szz())
                    : (void)0;
            }
            void* av = reinterpret_cast<void*>(new vector<typename bx<ii>::t>());
            aut& aa = *static_cast<vi*>(av);
            fr(i,0,ks) aa.pb(*reinterpret_cast<ii*>(&ss[i]));
            fr(i,0,(ii)bb.szz()) aa.pb(*reinterpret_cast<ii*>(&bb[i]));
            fr(i,ks,(ii)ss.szz()) aa.pb(*reinterpret_cast<ii*>(&ss[i]));
            fr(i,0,nn){
                pri<<*reinterpret_cast<ii*>(&aa[i])<<(i+1==nn?"":" ");
            }
            pri<<nl;
            delete static_cast<vi*>(sv);
            delete static_cast<vi*>(bv);
            delete static_cast<vi*>(av);
        }());
    delete static_cast<vi*>(pv);
}
...
void oox(){
    ...
    ii tt; iin>>tt;
    aut lm = [&](){ sl(); };
    fx< function<void()> > fn{ function<void()>(lm) };
    fr(i,0,tt) fn();
