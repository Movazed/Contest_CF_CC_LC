...
struct aa{
    virtual void* fx(void*,void*,void*,void*)=0;
    virtual ~aa(){}
};

struct bb:aa{
    void* fx(void* a,void* b,void*,void*){
        ii* x=reinterpret_cast<ii*>(a);
        ii* y=reinterpret_cast<ii*>(b);

        // single edge or domino decided by y == nullptr
        (!y)
        ? (cout<<*x<<" "<<*x<<"\n",nullptr)
        : (*y<0
            ? (cout<<*x<<" "<<-*y<<"\n",nullptr)
...
struct cc{
    aa* k;
    cc(aa* q):k(q){}
    void* operator()(void* a,void* b,void* c,void* d){
        return k->fx(a,b,c,d);
    }
};
...
int32_t main(){
    ...
    while(t--){
        ...
        if((x>y)||(x==0 && !(y&1))) continue;

        ii cr=2;

        auto se=[&](ii u,ii v){
            op(reinterpret_cast<void*>(&u),
