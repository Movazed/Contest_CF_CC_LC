...
template<class t>
struct b0{ virtual t f(void*)=0; virtual ~b0(){} };

template<class t>
struct b1:b0<t>{
    function<t(void*)> g;
    b1(function<t(void*)> _g):g(_g){}
    t f(void* x){ return g(x); }
};

struct b2{ void* p; b2(void* x):p(x){} void* g(){ return p; } };
...
void sol(){
    ...
    fr(i,0,n){
        ll x; cin>>x;
        *(reinterpret_cast<ll*>(a.data())+i)=x;
    }
    sort(all(a));
    aut lam = [&](void* x)->ll{
        return *reinterpret_cast<ll*>(x);
    };
    b0<ll>* o = new b1<ll>(lam);
    fr(i,0,n){
        ll* ap = reinterpret_cast<ll*>(a.data());
        ll cur = *(ap+i);
        (cur<=c) ?
        ([&](){
            ll use = min(k, c-cur);
